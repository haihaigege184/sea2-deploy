...
class PrintPlugin extends BasePlugin {
    ...
    async onMessage(context) {
        ...
        for (const cmd of this.commands.global) {
            if (cmd.match(ctx)) { return await cmd.handle(ctx); }
        }
    ...
    _registerCommands() {
        ...
        this.commands = {
            global: [
                ...
                { name: '撤回', match: c => c.msg === '开启撤回' || c.msg === '关闭撤回', handle: c => P._cmdRecall(c) },
                { name: '监听此群', match: c => c.msg === '监听此群' && (c.isSuper || c.isDev), handle: c => P._cmdListen(c, true) },
                { name: '不监听此群', match: c => c.msg === '不监听此群' && (c.isSuper || c.isDev), handle: c => P._cmdListen(c, false) },
                { name: '开启演示', match: c => c.msg === '开启演示' && (c.isSuper || c.isDev), handle: c => P._cmdDemo(c, true) },
                { name: '关闭演示', match: c => c.msg === '关闭演示' && (c.isSuper || c.isDev), handle: c => P._cmdDemo(c, false) },
