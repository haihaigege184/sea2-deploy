...
class PrintPlugin extends BasePlugin {
    ...
    async convertOfficeToPdf(filePath) {
        ...
        const userProfile = '/tmp/soffice_' + stamp;
        fs.mkdirSync(workDir, { recursive: true });
        let pdfPath = null;
        try {
            const cmd = 'soffice --headless -env:UserInstallation=file://' + userProfile +
    ...
    async _instantPrint(c) {
        const { msg, groupId, session, isDemo } = c;
        try {
            const result = await this.downloadFile(msg);
            await this.sendAndAutoRecall(groupId, '打印...', session.msgIds, 5000);
            if (result.isPdf) {
    ...
    async printFile(opts) {
        ...
        try {
            if (isOffice) {
                ...
                await execPromise(`img2pdf --rotation=ifvalid "${filePath}" -o "${tempPdf}"`, this.TO.proc);
                if (!fs.existsSync(tempPdf) || fs.statSync(tempPdf).size < 1024) {
                    throw new Error('PDF生成失败');
                }
                target = tempPdf;
