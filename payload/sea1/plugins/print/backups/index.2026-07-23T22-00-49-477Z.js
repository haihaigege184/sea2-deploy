...
class PrintPlugin extends BasePlugin {
    ...
    // ===== 测试入口结束 =====
        ...
        if (userMode && userMode.mode === 'a3' && (!a3Session || !a3Session.stage) &&
            (msg.includes('[CQ:image') || msg.includes('[CQ:file'))) {
            const session = a3Session || { msgIds: [], lastActive: Date.now(), cutJobs: [] };
            this.sessions.set(userId, session);
            ...
            try {
                ...
                for (let si = 0; si < sheets.length; si++) {
                    ...
                    if (parts === 0) {
                        session.stage = this.STAGE.WAITING_CUTS;
                    ...
                    if (sheets.length > 1) {
                        const acc = session.cutJobs.reduce(function (a, j) { return a + j.buffers.length; }, 0);
        ...
        if (userMode && userMode.mode === 'stitch' &&
            ...
            await this.sendAndAutoRecall(groupId, '已收 ' + userMode.images.length + ' 张', session.msgIds, 10000);
            return true;
        }
        ...
        if (userMode && userMode.mode === 'stitch' && msg === '完成') {
            ...
            session.stage = this.STAGE.WAITING_LAYOUT;
