/**
 * SEA1 插件：print - 智能打印服务
 * 移植狂打作业的A3切割算法
 */

const BasePlugin = require('../base-plugin');
const fs = require('fs');
const { exec } = require('child_process');
const util = require('util');
const path = require('path');
const http = require('http');
// 激活服务地址解析（scheme 无关，http/https 兼容，换域名不重装）
const { resolveActivationUrl } = require('../../lib/activation-url');
// 复用 vip_core 纯逻辑层（与插件契约无关）触发首印提醒，避免重复实现（见 system_design_activation_simplify.md §2）
const core = require('../vip/vip_core');

const execPromise = util.promisify(exec);

// ========== Office 文档转 PDF 支持 ==========
// LibreOffice 可转换的扩展名白名单（尽量全；wps/et/dps/mdb 视实际支持，失败返回明确错误）
const OFFICE_EXTS = ['.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx', '.odt', '.ods', '.odp', '.rtf', '.csv', '.txt', '.wps', '.et', '.dps', '.mdb'];
const OFFICE_SUPPORT_HINT = 'doc/docx/xls/xlsx/ppt/pptx/odt/ods/odp/rtf/csv/txt';

/** 判断文件名是否为 office 文档扩展名 */
function isOfficeExt(fileName) {
    if (!fileName) return false;
    const ext = path.extname(String(fileName).split('?')[0]).toLowerCase();
    return OFFICE_EXTS.indexOf(ext) !== -1;
}

class PrintPlugin extends BasePlugin {
    constructor(name, config) {
        super(name, config);
        this.priority = 30;
        this.permissionLevel = 0;
        
        this.printerName = "Default_Printer";
        this.tempDir = config.temp_dir || "/dev/shm";
        this.sessionTimeout = 5 * 60 * 1000;
        this.TO = { quick: 3000, net: 5000, mid: 15000, proc: 30000, long: 60000, xlong: 120000 };
        this.allowedGroups = config.notify_groups || [];
        this.recallEnabled = true;
        
        this.napcatHost = config.napcat_host;
        this.napcatPort = config.napcat_port || 3000;
        this.napcatToken = config.napcat_token || "";
        this.superAdmin = config.superAdmin || "";
        this.developer = config.developer || "";
        
        this.demoMode = config.demo_mode || {};
        
        this.AUTO_BLACK_CONFIG = "-lat 15x15-10% -normalize -auto-level";
        
        this.A4_W = 2480;
        this.A4_H = 3508;
        this.GAP = 59;
        this.MAX_COLS = 4;
        this.MAX_ROWS = 4;
        
        this.sessions = new Map();
        // Office 转换：warm profile 池（复用 soffice 初始化，约省 40% 耗时）+ 并发信号量（避免 profile 锁冲突）
        this._officeProfilePool = ['/tmp/sea2_soffice_p0', '/tmp/sea2_soffice_p1'];
        this._officeBusy = this._officeProfilePool.map(() => false);
        this._officeWaiters = [];
        try { this._officeProfilePool.forEach(p => fs.mkdirSync(p, { recursive: true })); } catch (e) {}
        // 极速打印批量收集（支持一条消息多文件 + 瞬间连发合并为一批）
        this._instantBatches = new Map();
        this.userMsgCache = new Map();
        this.configPath = path.join(__dirname, '../../config.json');
        // 激活服务地址来源（license 子配置中的 activationServer；解析见 _actServer）
        this._licenseCfg = {};
        // [打印门禁] 权限判定缓存（userId -> {allow, expireAt}）+ 豁免群通知节流（groupId -> ts）
        this._printPermCache = new Map();
        this._graceNotifyAt = new Map();
        
        this.SUB = {
            CUTS: 'WAITING_CUTS',
            PRINT: 'WAITING_PRINT',
            LAYOUT: 'WAITING_LAYOUT',
            COMPLETED: 'COMPLETED'
        };
        this.commands = { global: [], allowed: [], modeEntry: [] };
        this.modeDispatchers = {};
        this._registerCommands();

        
        this.cleanupInterval = null;
        this.queueSession = null;
    }

    async onEnable(db) {
        console.log(`[Print] ========================================`);
        console.log(`[Print] 智能打印插件 v5.8 加载中...`);
        
        await this.loadConfig();
        
        console.log(`[Print] 打印机: ${this.printerName}`);
        console.log(`[Print] NapCat: ${this.napcatHost}:${this.napcatPort}`);
        console.log(`[Print] 监听群组: ${this.allowedGroups.join(', ') || '全部'}`);
        console.log(`[Print] 撤回功能: ${this.recallEnabled ? '开启' : '关闭'}`);
        
        try {
            await execPromise('convert -version', this.TO.quick);
            console.log(`[Print] ImageMagick: OK`);
        } catch (e) {
            console.log(`[Print] ImageMagick: 未安装`);
        }
        
        try {
            await execPromise('pdftoppm -v', this.TO.quick);
            console.log(`[Print] pdftoppm: OK`);
        } catch (e) {
            console.log(`[Print] pdftoppm: 未安装`);
        }

        try {
            await execPromise('soffice --version', this.TO.quick);
            console.log(`[Print] LibreOffice: OK`);
        } catch (e) {
            console.log(`[Print] LibreOffice: 未安装（office 文档转 PDF 不可用）`);
        }
        
        if (!fs.existsSync(this.tempDir)) {
            fs.mkdirSync(this.tempDir, { recursive: true });
        }
        
        if (this.cleanupInterval) {
            clearInterval(this.cleanupInterval);
        }
        this.cleanupInterval = setInterval(this.cleanupSessions.bind(this), 60000);
        
        console.log(`[Print] 加载完成`);
        console.log(`[Print] ========================================`);
    }

    async onDisable(db) {
        if (this.cleanupInterval) {
            clearInterval(this.cleanupInterval);
            this.cleanupInterval = null;
        }
        console.log(`[Print] 已卸载`);
    }

    cleanupSessions() {
        const now = Date.now();
        if (this.sessions) {
            for (const [userId, session] of this.sessions.entries()) {
                if (now - session.lastActive > this.sessionTimeout) {
                    this.sessions.delete(userId);
                }
            }
        }
    }

    // ==========================================
    // 配置管理
    // ==========================================

    async loadConfig() {
        try {
            const configData = fs.readFileSync(this.configPath, 'utf-8');
            const config = JSON.parse(configData);
            
            if (config.printer && config.printer.default) {
                this.printerName = config.printer.default;
            }
            if (config.notify_groups) {
                this.allowedGroups = config.notify_groups;
            }
            if (config.napcat_host) {
                this.napcatHost = config.napcat_host;
            }
            if (config.napcat_port) {
                this.napcatPort = config.napcat_port;
            }
            if (config.napcat_token) {
                this.napcatToken = config.napcat_token;
            }
            if (config.superAdmin) {
                this.superAdmin = config.superAdmin;
            }
            if (config.developer) {
                this.developer = config.developer;
            }
            if (config.demo_mode) {
                this.demoMode = config.demo_mode;
            }
            if (config.license) {
                this._licenseCfg = config.license;
            }
            if (config.recall_enabled !== undefined) {
                this.recallEnabled = config.recall_enabled;
            }
        } catch (e) {
            console.log(`[Print] 读取配置失败: ${e.message}`);
        }
    }

    async saveConfig() {
        try {
            const configData = fs.readFileSync(this.configPath, 'utf-8');
            const config = JSON.parse(configData);
            
            config.printer = config.printer || {};
            config.printer.default = this.printerName;
            config.notify_groups = this.allowedGroups;
            config.demo_mode = this.demoMode;
            config.napcat_host = this.napcatHost;
            config.napcat_port = this.napcatPort;
            config.napcat_token = this.napcatToken;
            config.recall_enabled = this.recallEnabled;
            
            fs.writeFileSync(this.configPath, JSON.stringify(config, null, 2), 'utf-8');
            console.log(`[Print] 配置已保存`);
            return true;
        } catch (e) {
            console.log(`[Print] 保存配置失败: ${e.message}`);
            return false;
        }
    }

    // ==========================================
    // 权限检查
    // ==========================================

    isSuperAdmin(userId) {
        try {
            const perm = global.sea1 && global.sea1.permission;
            if (perm && typeof perm.hasLevelSync === 'function') {
                return perm.hasLevelSync(String(userId), 2);
            }
        } catch (e) { /* 权限库不可用走 config 兜底 */ }
        return String(userId) === String(this.superAdmin);
    }

    isDeveloper(userId) {
        try {
            const perm = global.sea1 && global.sea1.permission;
            if (perm && typeof perm.hasLevelSync === 'function') {
                return perm.hasLevelSync(String(userId), 3);
            }
        } catch (e) { /* 权限库不可用走 config 兜底 */ }
        return String(userId) === String(this.developer) || this.isSuperAdmin(userId);
    }

    // ==========================================
    // 群组管理
    // ==========================================

    isGroupAllowed(groupId) {
        if (!groupId) return true;
        if (this.allowedGroups.length === 0) return true;
        return this.allowedGroups.includes(String(groupId));
    }

    isDemoMode(groupId) {
        return this.demoMode[String(groupId)] === true;
    }

    /**
     * 解析激活服务地址（与 vip 插件同源：env SEA1_ACTIVATION_URL > config.license.activationServer > 兜底）。
     * 首印提醒需要回查用户试用状态并标记去重，故 print 插件需能拿到激活服务器地址。
     * @returns {string}
     */
    _actServer() {
        return resolveActivationUrl(this._licenseCfg);
    }

    /**
     * 打印完成钩子：触发「首印提醒」（激活流程简化 P0-5）。
     * 仅当该用户试用期已到期且首印提醒标记未置位时，发送一次温柔提醒（去重，落盘于激活服务端）。
     * 该提醒为普通文本回复，不触发 vip 功能门禁（vip 已在 licensing 只读白名单）。
     * 非阻塞：以 best-effort 调用，异常被吞掉，绝不影响打印主流程。
     *
     * @param {Object} c 打印命令上下文（含 userId、context.reply）
     */
    _afterPrintDone(c) {
        try {
            const { userId, context } = c;
            if (!userId || userId === '0' || !context) return;
            const server = this._actServer();
            if (!server) return;
            const monitorToken = process.env.MONITOR_TOKEN || (this._licenseCfg && this._licenseCfg.monitorToken) || '';
            // 调 vip_core 纯逻辑层：判定是否需提醒（不 await，避免阻塞打印回执）
            core.maybeTrialExpiredRemind(server, userId)
                .then((res) => {
                    if (res && res.reminded && res.lines && res.lines.length) {
                        context.reply(res.lines.join('\n'))
                            .catch((e) => console.warn('[Print] 首印提醒发送失败（已忽略）:', (e && e.message) || e));
                        // 标记已提醒（去重），best-effort
                        core.markExpiredPrintReminded(server, userId, monitorToken)
                            .catch((e) => console.warn('[Print] 首印提醒标记失败（已忽略）:', (e && e.message) || e));
                    }
                })
                .catch((e) => console.warn('[Print] 首印提醒查询异常（已忽略）:', (e && e.message) || e));
        } catch (e) {
            console.warn('[Print] 首印提醒异常（已忽略）:', (e && e.message) || e);
        }
    }

    /**
     * [打印门禁] 服务端不可达豁免通知（按群节流，10 分钟最多一次）。
     * @param {string} groupId
     */
    async _notifyGraceMode(groupId) {
        try {
            const now = Date.now();
            const last = this._graceNotifyAt.get(groupId) || 0;
            if (now - last < 10 * 60 * 1000) return;
            this._graceNotifyAt.set(groupId, now);
            await this.sendGroupMessage(groupId,
                '⚠️ 服务端暂时不可达，本次打印按豁免策略放行\n' +
                '（与会员到期豁免期同策略，服务端恢复后自动恢复校验）'
            );
        } catch (e) {
            console.warn('[Print] 豁免通知失败（已忽略）:', (e && e.message) || e);
        }
    }

    /**
     * [打印门禁] 打印权限校验：先连服务端获取试用/会员资格，再放行打印。
     * - 超管/开发者：直接放行
     * - 服务端有记录且 active/converted（试用中或正式会员）：放行（结果缓存 10 分钟）
     * - 无记录（首触）：调服务端授予账号维度试用，授予成功才放行（保证首触试用成功识别）
     * - 已到期（expired）：拒绝并引导开通会员
     * - 服务端不可达/响应异常：豁免放行 + 群内通知（节流），与会员到期豁免期同策略
     * @param {Object} c 打印命令上下文（含 userId/groupId/isSuper/isDev）
     * @returns {{allow:boolean, lines?:string[]}}
     */
    async _checkPrintPerm(c) {
        const { userId, groupId, isSuper, isDev } = c;
        if (isSuper || isDev) return { allow: true };
        const now = Date.now();
        const cached = this._printPermCache.get(userId);
        if (cached && cached.expireAt > now && cached.allow) return { allow: true };

        const server = this._actServer();
        if (!server) return { allow: true }; // 未配置激活服务（纯单机）→ 不设门禁
        const monitorToken = process.env.MONITOR_TOKEN || (this._licenseCfg && this._licenseCfg.monitorToken) || '';

        let r = null;
        try { r = await core.trialStatusByUin(server, userId); } catch (e) { r = null; }
        const b = r && r.body;

        if (!b || b.ok !== true) {
            // 服务端不可达或响应异常 → 豁免放行 + 群内通知
            console.warn('[Print] 门禁: 服务端不可达，豁免放行 uin=' + userId);
            await this._notifyGraceMode(groupId);
            return { allow: true };
        }

        if (b.exists && (b.status === 'active' || b.status === 'converted')) {
            this._printPermCache.set(userId, { allow: true, expireAt: now + 10 * 60 * 1000 });
            return { allow: true };
        }

        if (b.exists && b.status === 'expired') {
            return { allow: false, lines: core.reminderText() };
        }

        if (!b.exists) {
            // 首触：必须先在服务端登记试用权，成功才放行打印
            let months = 3;
            try {
                const cfg = await core.getTrialConfig(server);
                if (cfg && cfg.body && cfg.body.ok && cfg.body.months) months = cfg.body.months;
            } catch (e) { /* 用默认 3 个月 */ }
            let g = null;
            try { g = await core.grantTrial(server, { uin: userId, months }, monitorToken); } catch (e) { g = null; }
            if (g && g.body && g.body.ok) {
                console.log('[Print] 门禁: 已为新用户 ' + userId + ' 授予 ' + months + ' 个月试用（首触）');
                this._printPermCache.set(userId, { allow: true, expireAt: now + 10 * 60 * 1000 });
                return { allow: true };
            }
            // 授予失败（试用已关闭/服务端异常）→ 视为不可达，走豁免
            console.warn('[Print] 门禁: 首触授予失败，豁免放行 uin=' + userId);
            await this._notifyGraceMode(groupId);
            return { allow: true };
        }

        // 其他未知状态 → 豁免放行（宁纵勿枉，避免误伤会员）
        await this._notifyGraceMode(groupId);
        return { allow: true };
    }

    /**
     * 权限中间件：统一校验命令所需的权限级别
     * @param {Object} ctx 命令上下文（含 isSuper/isDev）
     * @param {number} [level=0] 所需权限级别：0=所有人，2=需超管或开发者
     * @returns {boolean} 是否放行
     */
    _authorize(ctx, level = 0) {
        if (level <= 0) return true;
        if (level >= 2) return !!(ctx.isSuper || ctx.isDev);
        return true;
    }

    async addGroup(groupId) {
        const gid = String(groupId);
        if (!this.allowedGroups.includes(gid)) {
            this.allowedGroups.push(gid);
            await this.saveConfig();
            return true;
        }
        return false;
    }

    async removeGroup(groupId) {
        const gid = String(groupId);
        const index = this.allowedGroups.indexOf(gid);
        if (index !== -1) {
            this.allowedGroups.splice(index, 1);
            await this.saveConfig();
            return true;
        }
        return false;
    }

    async enableDemo(groupId) {
        const gid = String(groupId);
        this.demoMode[gid] = true;
        await this.saveConfig();
        return true;
    }

    async disableDemo(groupId) {
        const gid = String(groupId);
        delete this.demoMode[gid];
        await this.saveConfig();
        return true;
    }

    // ==========================================
    // HTTP API
    // ==========================================

    getHeaders() {
        const headers = { 'Content-Type': 'application/json' };
        if (this.napcatToken) {
            headers['Authorization'] = `Bearer ${this.napcatToken}`;
        }
        return headers;
    }

    sendGroupMessage(groupId, text) {
        return new Promise((resolve) => {
            const postData = JSON.stringify({
                group_id: parseInt(groupId),
                message: text
            });
            const options = {
                hostname: this.napcatHost,
                port: this.napcatPort,
                path: '/send_group_msg',
                method: 'POST',
                headers: this.getHeaders(),
                timeout: this.TO.net
            };
            const req = http.request(options, (res) => {
                let data = '';
                res.on('data', chunk => data += chunk);
                res.on('end', () => {
                    try {
                        const result = JSON.parse(data);
                        if (result.status === 'ok' && result.data && result.data.message_id) {
                            resolve(result.data.message_id);
                        } else {
                            resolve(null);
                        }
                    } catch(e) {
                        resolve(null);
                    }
                });
            });
            req.on('error', () => resolve(null));
            req.on('timeout', () => { req.destroy(); resolve(null); });
            req.write(postData);
            req.end();
        });
    }

    async recallMessage(messageId) {
        if (!this.recallEnabled) return false;
        if (!messageId) return false;
        try {
            const postData = JSON.stringify({ message_id: parseInt(messageId) });
            const options = {
                hostname: this.napcatHost,
                port: this.napcatPort,
                path: '/delete_msg',
                method: 'POST',
                headers: this.getHeaders(),
                timeout: this.TO.net
            };
            return new Promise((resolve) => {
                const req = http.request(options, (res) => {
                    let data = '';
                    res.on('data', chunk => data += chunk);
                    res.on('end', () => {
                        try {
                            const result = JSON.parse(data);
                            resolve(result.retcode === 0);
                        } catch(e) {
                            resolve(false);
                        }
                    });
                });
                req.on('error', () => resolve(false));
                req.on('timeout', () => { req.destroy(); resolve(false); });
                req.write(postData);
                req.end();
            });
        } catch (e) {
            return false;
        }
    }

    getUserMessageId(context) {
        if (context.raw && context.raw.message_id) {
            return context.raw.message_id;
        }
        if (context.message_id) {
            return context.message_id;
        }
        return null;
    }

    recordUserMessage(userId, messageId) {
        if (!messageId) return;
        if (!this.userMsgCache.has(userId)) {
            this.userMsgCache.set(userId, []);
        }
        this.userMsgCache.get(userId).push(messageId);
    }

    async recallUserMessages(userId) {
        if (!this.recallEnabled) return;
        const ids = this.userMsgCache.get(userId) || [];
        for (const id of ids) {
            await this.recallMessage(id);
        }
        this.userMsgCache.delete(userId);
    }

    async sendAndAutoRecall(groupId, text, msgIds, delay = 10000) {
        const msgId = await this.sendGroupMessage(groupId, text);
        if (msgId && msgIds && this.recallEnabled) {
            msgIds.push(msgId);
            setTimeout(() => {
                this.recallMessage(msgId);
            }, delay);
        }
        return msgId;
    }

    async sendPreview(groupId, imageBuffer, text) {
        const cqImage = `[CQ:image,file=base64://${imageBuffer.toString('base64')}]`;
        return await this.sendGroupMessage(groupId, cqImage + '\n' + text);
    }

    // ==========================================
    // 队列功能
    // ==========================================

    async getPrintQueue(printerName) {
        try {
            const result = await execPromise(`lpstat -o "${printerName}" 2>&1`, this.TO.mid);
            return result.stdout ? result.stdout : result.toString();
        } catch (e) {
            return '';
        }
    }

    formatQueuePretty(output) {
        if (!output || output.includes('no entries')) return null;
        
        const lines = output.split('\n');
        const jobs = [];
        
        for (const line of lines) {
            const match = line.match(/^(\S+)-(\d+)\s+\S+\s+(\d+)\s+(.+)$/);
            if (match) {
                const jobNum = match[2];
                const sizeBytes = parseInt(match[3]);
                const timeRaw = match[4];
                
                let sizeStr;
                if (sizeBytes >= 1048576) sizeStr = (sizeBytes / 1048576).toFixed(1) + 'MB';
                else if (sizeBytes >= 1024) sizeStr = Math.round(sizeBytes / 1024) + 'KB';
                else sizeStr = sizeBytes + 'B';
                
                let timeStr = timeRaw;
                try {
                    const d = new Date(timeRaw);
                    if (!isNaN(d.getTime())) {
                        timeStr = d.toLocaleTimeString('zh-CN', { hour12: false });
                    }
                } catch(e) {}
                
                jobs.push({ num: jobNum, size: sizeStr, time: timeStr });
            }
        }
        
        if (jobs.length === 0) return null;
        
        return [
            '[打印队列]',
            '-------------------',
            '共 ' + jobs.length + ' 条打印任务',
            '-------------------',
            ...jobs.map(j => '序号 ' + j.num + ' | ' + j.size + ' | ' + j.time),
            '-------------------',
            '[√] 回复数字取消对应任务',
            '[√] 回复 0 取消全部任务',
            '[×] 回复 q 退出'
        ].join('\n');
    }

    async cancelPrintJob(jobId) {
        try {
            await execPromise(`cancel "${jobId}"`, this.TO.mid);
            return true;
        } catch (e) {
            return false;
        }
    }

    async cancelAllJobs(printerName) {
        try {
            const out = await this.getPrintQueue(printerName);
            const lines = out.trim().split('\n').filter(l => l && !l.includes('no entries'));
            let cancelled = 0;
            for (const line of lines) {
                const match = line.match(/^(\S+-\d+)/);
                if (match) {
                    try { 
                        await execPromise(`cancel "${match[1]}"`, this.TO.mid); 
                        cancelled++; 
                    } catch (e) {}
                }
            }
            return cancelled;
        } catch (e) {
            return 0;
        }
    }

    // ==========================================
    // 狂打A3切割算法（移植）
    // ==========================================

    /**
     * ============================================================
     * A3 中缝智能检测 v2（增强版）
     * 解决：密封区干扰 / 透视失真 / 非标准版面
     * ============================================================
     */
    async findMidGap(filePath, imgW, imgH) {
        try {
            const isWidthLonger = imgW > imgH;
            const longSide = isWidthLonger ? imgW : imgH;
            const shortSide = isWidthLonger ? imgH : imgW;

            // 全高采样：取整条高度的竖线，不再只取中间20%
            const sampleWidth = shortSide;
            const sampleOffset = 0;

            const densities = [];

            for (let pct = 25; pct <= 75; pct++) {
                const pos = Math.floor(longSide * pct / 100);
                let cropGeom;
                if (isWidthLonger) {
                    cropGeom = `1x${sampleWidth}+${pos}+${sampleOffset}`;
                } else {
                    cropGeom = `${sampleWidth}x1+${sampleOffset}+${pos}`;
                }
                try {
                    const cmd = `convert "${filePath}" -crop ${cropGeom} +repage -threshold 50% -negate -format "%[fx:mean*w*h]" info:`;
                    const result = await execPromise(cmd, this.TO.proc);
                    const outStr = (typeof result === 'string') ? result : (result.stdout || '');
                    const blackPixels = parseFloat(outStr.trim());
                    if (!isNaN(blackPixels)) {
                        densities.push({ pct, density: blackPixels });
                    }
                } catch (e) {}
            }
            if (densities.length === 0) return { positions: [], confidence: 0 };
            let valleys = [];
            for (let i = 2; i < densities.length - 2; i++) {
                if (densities[i].density === 0) valleys.push({ pct: densities[i].pct, density: 0 });
            }
            // 保留全部空白竖线（不再只取最中间一条），供二开/三开判断
            const filtered = [];
            for (const v of valleys) {
                if (filtered.length === 0 || v.pct - filtered[filtered.length - 1].pct >= 10) {
                    filtered.push(v);
                }
            }
            const positions = filtered.map(v => v.pct / 100);
            const confidence = positions.length > 0 ? 1.0 : 0;
            console.log('[Print] 中缝检测(全高): 候选' + filtered.length + '个 pos=' + positions.map(function (p) { return (p*100).toFixed(1)+'%'; }).join(',') + ' conf=' + confidence.toFixed(2));
            return { positions, confidence };
        } catch (e) {
            console.log('[Print] findMidGap错误: ' + e.message);
            return { positions: [], confidence: 0 };
        }
    }




    /**
     * 回退中缝检测（单行梯度法，处理极端情况）
     * 在顶部行、中间行、底部行各取一条，求共同低密度列
     */
    async _fallbackSeamDetection(filePath, imgW, imgH) {
        try {
            const isWidthLonger = imgW > imgH;
            const LONG = isWidthLonger ? imgW : imgH;
            const SHORT = isWidthLonger ? imgH : imgW;

            // 顶部行（跳过密封区）、中间行、底部行
            const rows = [
                Math.floor(SHORT * 0.20),  // 跳过密封区
                Math.floor(SHORT * 0.50),  // 中间
                Math.floor(SHORT * 0.80)  // 底部
            ];

            const densities = [];
            for (const row of rows) {
                const rowDens = [];
                for (let pct = 20; pct <= 80; pct++) {
                    const pos = Math.floor(LONG * pct / 100);
                    let cropGeom = isWidthLonger
                        ? `1x${Math.floor(SHORT * 0.2)}+${pos}+${Math.floor(SHORT * 0.4)}`
                        : `${Math.floor(SHORT * 0.2)}x1+${Math.floor(SHORT * 0.4)}+${pos}`;
                    try {
                        const cmd = `convert "${filePath}" -crop ${cropGeom} +repage -threshold 50% -negate -format "%[fx:mean*w*h]" info:`;
                        const r = await execPromise(cmd, this.TO.proc);
                        const v = parseFloat((r.stdout || '').trim());
                        rowDens.push({ pct, density: isNaN(v) ? 1 : v });
                    } catch(e) { rowDens.push({ pct, density: 1 }); }
                }
                densities.push(rowDens);
            }

            // 对每列计算跨行一致性（所有行密度都很低的列）
            const scores = [];
            for (let i = 0; i < densities[0].length; i++) {
                const avg = densities.reduce((s, r) => s + (r[i]?.density || 1), 0) / densities.length;
                if (avg < 0.15) {
                    // 计算梯度（与相邻列的差值）
                    const left = i > 0 ? densities.reduce((s, r) => s + (r[i-1]?.density || 1), 0) / densities.length : 1;
                    const right = i < densities[0].length-1 ? densities.reduce((s, r) => s + (r[i+1]?.density || 1), 0) / densities.length : 1;
                    scores.push({ pct: densities[0][i].pct, avg, gradient: Math.min(left - avg, right - avg) });
                }
            }

            if (scores.length === 0) return { positions: [], confidence: 0 };

            // 合并相邻列（聚类）
            scores.sort((a, b) => a.pct - b.pct);
            const clusters = [];
            for (const s of scores) {
                if (clusters.length === 0 || s.pct - clusters[clusters.length-1].pct >= 3) {
                    clusters.push({ pcts: [s.pct], minAvg: s.avg, maxGrad: s.gradient });
                } else {
                    clusters[clusters.length-1].pcts.push(s.pct);
                    clusters[clusters.length-1].minAvg = Math.min(clusters[clusters.length-1].minAvg, s.avg);
                    clusters[clusters.length-1].maxGrad = Math.max(clusters[clusters.length-1].maxGrad, s.gradient);
                }
            }

            // 取密度最低的代表
            const candidates = clusters.map(cl => {
                const bestPct = cl.pcts[Math.floor(cl.pcts.length / 2)];
                return { pct: bestPct / 100, density: cl.minAvg, gradient: cl.maxGrad };
            });

            // 过滤密封区（只在顶部有差异）
            const valid = candidates.filter(c => {
                // 检查这列是否在各行都空白
                for (const rowDens of densities) {
                    const item = rowDens.find(d => Math.abs(d.pct - c.pct * 100) < 2);
                    if (item && item.density > 0.25) return false;
                }
                return true;
            });

            const final = valid.length > 0 ? valid : candidates;
            const best = final.reduce((a, b) => Math.abs(a.pct - 0.5) < Math.abs(b.pct - 0.5) ? a : b);
            const confidence = (1 - best.density) * 0.6 + Math.min(best.gradient, 0.5) * 0.4;

            console.log(`[Print] 回退检测: pos=${(best.pct*100).toFixed(1)}% conf=${(confidence||0).toFixed(2)}`);
            return { positions: [best.pct], confidence: Math.min(confidence || 0, 1) };
        } catch (e) {
            console.log(`[Print] _fallbackSeamDetection错误: ${e.message}`);
            return { positions: [0.5], confidence: 0.5 };
        }
    }

    /**
     * 按中缝位置智能切割（支持透视校正）
     * 若中缝倾斜（透视失真），先做斜线裁切再等宽切割
     */
    async splitImageByPosition(filePath, direction, positions) {
        const outputFiles = [];
        let imgW = 0, imgH = 0;
        try {
            const sizeOut = await execPromise('identify -format "%w %h" "' + filePath + '"', this.TO.mid);
            let dims = (sizeOut.stdout || '').trim();
            if (!dims) dims = (sizeOut.stderr || '').trim();
            if (!dims) dims = sizeOut.toString().trim();
            const parts = dims.split(/\s+/).map(Number);
            imgW = parts[0]; imgH = parts[1];
        } catch (e) {
            console.log('[Print] identify失败: ' + e.message);
        }
        if (!imgW || !imgH || isNaN(imgW) || isNaN(imgH)) {
            throw new Error('无法获取图片尺寸: ' + filePath);
        }
        const cuts = [0].concat(positions, [1]);
        for (let i = 0; i < cuts.length - 1; i++) {
            let outFile;
            if (/\.\w+$/.test(filePath)) outFile = filePath.replace(/\.\w+$/, '_part' + i + '.jpg');
            else outFile = filePath + '_part' + i + '.jpg';
            const startPct = cuts[i], endPct = cuts[i + 1];
            if (direction === 'vertical') {
                const x = Math.round(startPct * imgW);
                const w = Math.round((endPct - startPct) * imgW);
                await execPromise('convert "' + filePath + '" -crop ' + w + 'x' + imgH + '+' + x + '+0 +repage "' + outFile + '"', this.TO.proc);
            } else {
                const y = Math.round(startPct * imgH);
                const h = Math.round((endPct - startPct) * imgH);
                await execPromise('convert "' + filePath + '" -crop ' + imgW + 'x' + h + '+0+' + y + ' +repage "' + outFile + '"', this.TO.proc);
            }
            outputFiles.push(outFile);
        }
        return outputFiles;
    }



    /**
     * 智能判断两开/三开（增强版）
     * 综合：位置比例 + 密度对比 + 边缘分布
     */
    async detectParts(filePath, imgW, imgH) {
        const gap = await this.findMidGap(filePath, imgW, imgH);
        if (gap.positions.length === 0) return { parts: 0, positions: [], confidence: 0 };
        const sorted = gap.positions.slice().sort(function (a, b) { return a - b; });
        // 三开：2条缝分别在 33%/67% 附近
        if (sorted.length >= 2) {
            const p1 = sorted[0], p2 = sorted[sorted.length - 1];
            if (Math.abs(p1 - 0.333) < 0.12 && Math.abs(p2 - 0.667) < 0.12) {
                console.log('[Print] 判断三开: ' + (p1*100).toFixed(1) + '%, ' + (p2*100).toFixed(1) + '%');
                return { parts: 3, positions: [p1, p2], confidence: gap.confidence * 0.9 };
            }
        }
        // 二开：恰好1条缝且靠近中间
        if (sorted.length === 1 && Math.abs(sorted[0] - 0.5) < 0.15) {
            return { parts: 2, positions: [sorted[0]], confidence: gap.confidence * 0.85 };
        }
        // 多缝但不符三开 / 单缝偏边 → 手动指定
        console.log('[Print] 中缝不符合二开/三开，转手动: ' + sorted.map(function (p) { return (p*100).toFixed(1)+'%'; }).join(','));
        return { parts: 0, positions: [], confidence: 0 };
    }



    // ==========================================
    // PDF 渲染
    // ==========================================

    async renderPdfToImage(pdfBuffer, pageNum = 1) {
        const pdfPath = path.join(this.tempDir, `pdf_${Date.now()}.pdf`);
        const outPrefix = path.join(this.tempDir, `pdf_page_${Date.now()}`);
        
        try {
            fs.writeFileSync(pdfPath, pdfBuffer);
            const cmd = `pdftoppm -png -r 200 -f ${pageNum} -l ${pageNum} "${pdfPath}" "${outPrefix}"`;
            console.log(`[Print] 渲染PDF: ${cmd}`);
            await execPromise(cmd, this.TO.long);
            
            const files = fs.readdirSync(this.tempDir);
            const pattern = new RegExp(path.basename(outPrefix) + '.*\\.png$');
            const matched = files.find(f => pattern.test(f));
            
            if (matched) {
                return path.join(this.tempDir, matched);
            } else {
                throw new Error('未找到渲染后的图片');
            }
        } catch (e) {
            console.log(`[Print] PDF渲染失败: ${e.message}`);
            throw e;
        } finally {
            try { if (fs.existsSync(pdfPath)) fs.unlinkSync(pdfPath); } catch(e) {}
        }
    }

    async renderPdfToImages(pdfBuffer) {
        const pdfPath = path.join(this.tempDir, `pdf_${Date.now()}.pdf`);
        const outPrefix = path.join(this.tempDir, `pdf_pages_${Date.now()}`);
        try {
            fs.writeFileSync(pdfPath, pdfBuffer);
            const cmd = `pdftoppm -png -r 200 "${pdfPath}" "${outPrefix}"`;
            console.log(`[Print] 渲染PDF(多页): ${cmd}`);
            await execPromise(cmd, this.TO.xlong);
            const base = path.basename(outPrefix);
            const matched = fs.readdirSync(this.tempDir)
                .filter(f => f.startsWith(base) && f.endsWith('.png'))
                .sort((a, b) => {
                    const na = parseInt((a.match(/-(\d+)\.png$/) || [])[1] || '0', 10);
                    const nb = parseInt((b.match(/-(\d+)\.png$/) || [])[1] || '0', 10);
                    return na - nb;
                })
                .map(f => path.join(this.tempDir, f));
            return matched;
        } catch (e) {
            console.log(`[Print] PDF多页渲染失败: ${e.message}`);
            throw e;
        } finally {
            try { if (fs.existsSync(pdfPath)) fs.unlinkSync(pdfPath); } catch(e) {}
        }
    }

    // ==========================================
    // 工具函数
    // ==========================================

    async downloadFile(message) {
        const imageMatch = message.match(/\[CQ:image,file=([^,\]]+)(?:,url=([^\]]+))?\]/);
        const fileMatch = message.match(/\[CQ:file,file=([^,\]]+)(?:,url=([^\]]+))?\]/);
        
        let fileUrl = null;
        let fileName = null;
        let isPdf = false;
        let isOffice = false;
        let fileExt = '';
        
        if (imageMatch) {
            fileName = imageMatch[1];
            fileUrl = imageMatch[2] || null;
            isPdf = false;
            isOffice = false;
        } else if (fileMatch) {
            fileName = fileMatch[1];
            fileUrl = fileMatch[2] || null;
            fileExt = path.extname(String(fileName).split('?')[0]).toLowerCase();
            isPdf = fileExt === '.pdf';
            isOffice = !isPdf && OFFICE_EXTS.indexOf(fileExt) !== -1;
        } else {
            throw new Error('无法解析文件信息');
        }
        
        if (fileUrl) {
            const controller = new AbortController();
            const timer = setTimeout(() => controller.abort(), this.TO.proc);
            let response;
            try {
                response = await fetch(fileUrl, { signal: controller.signal });
            } catch(fetchErr) {
                clearTimeout(timer);
                if (fetchErr.name === 'AbortError') throw new Error('下载超时 (30s)');
                throw fetchErr;
            }
            clearTimeout(timer);
            if (!response.ok) {
                throw new Error(`下载失败: HTTP ${response.status}`);
            }
            const arrayBuffer = await response.arrayBuffer();
            const buffer = Buffer.from(arrayBuffer);
            // office 文档：先转 PDF 再返回（调用方统一按 PDF 流程打印）
            if (isOffice) {
                const officePath = path.join(this.tempDir, 'office_in_' + Date.now() + '_' + Math.random().toString(16).slice(2, 8) + fileExt);
                fs.writeFileSync(officePath, buffer);
                let pdfPath = null;
                try {
                    pdfPath = await this.convertOfficeToPdf(officePath);
                    const pdfBuffer = fs.readFileSync(pdfPath);
                    if (!pdfBuffer || pdfBuffer.length < 1024) {
                        throw new Error('无法转换该文档：生成的 PDF 无效（' + fileName + '），支持 ' + OFFICE_SUPPORT_HINT);
                    }
                    return { buffer: pdfBuffer, isPdf: true, isOffice: true, fileName };
                } finally {
                    try { if (fs.existsSync(officePath)) fs.unlinkSync(officePath); } catch (e) {}
                    try { if (pdfPath) fs.rmSync(path.dirname(pdfPath), { recursive: true, force: true }); } catch (e) {}
                }
            }
            return { buffer, isPdf, isOffice, fileName };
        }
        
        throw new Error('无法获取文件下载链接');
    }

    /**
     * 用 LibreOffice headless 将 office 文档转换为 PDF。
     * - 独立 UserInstallation 隔离 profile，避免与已有 soffice 实例/并发转换冲突；
     * - 输出到独立临时目录，转换成功后校验 PDF 存在且 size>1024；
     * - 转换失败抛明确错误（含文件名+原因）。
     * @param {string} filePath 源文档路径
     * @returns {Promise<string>} 生成的 PDF 路径（位于独立临时目录，调用方用后需清理）
     */
    // Office 转换并发信号量：从 warm profile 池取一个空闲槽，无空闲则排队
    async _acquireOfficeProfile() {
        if (!this._officeBusy || !this._officeBusy.length) return -1;
        for (let i = 0; i < this._officeBusy.length; i++) {
            if (!this._officeBusy[i]) { this._officeBusy[i] = true; return i; }
        }
        return new Promise((resolve) => { this._officeWaiters.push(resolve); });
    }

    _releaseOfficeProfile(slot) {
        if (slot === -1 || !this._officeBusy || !this._officeBusy.length) return;
        this._officeBusy[slot] = false;
        if (this._officeWaiters && this._officeWaiters.length) {
            const resolve = this._officeWaiters.shift();
            for (let i = 0; i < this._officeBusy.length; i++) {
                if (!this._officeBusy[i]) { this._officeBusy[i] = true; resolve(i); return; }
            }
            this._officeWaiters.unshift(resolve);
        }
    }

    async convertOfficeToPdf(filePath) {
        const stamp = Date.now() + '_' + Math.random().toString(16).slice(2, 8);
        const workDir = path.join(this.tempDir, 'office_conv_' + stamp);
        fs.mkdirSync(workDir, { recursive: true });
        let pdfPath = null;
        const slot = await this._acquireOfficeProfile();
        const userProfile = slot >= 0 ? this._officeProfilePool[slot] : ('/tmp/soffice_' + stamp);
        try {
            const cmd = 'soffice --headless --nologo --norestore --nofirststartwizard -env:UserInstallation=file://' + userProfile +
                ' --convert-to pdf --outdir "' + workDir + '" "' + filePath + '"';
            try {
                await execPromise(cmd, { timeout: this.TO.xlong, maxBuffer: 64 * 1024 * 1024 });
            } catch (e) {
                throw new Error('无法转换该文档：' + (e.message || 'soffice 执行失败') + '，支持 ' + OFFICE_SUPPORT_HINT);
            }
            const baseName = path.basename(filePath).replace(/\.[^.]+$/, '');
            const outFile = path.join(workDir, baseName + '.pdf');
            if (fs.existsSync(outFile) && fs.statSync(outFile).size >= 1024) {
                pdfPath = outFile;
            } else {
                // 部分格式（如 mdb/wps）不产出预期文件名，扫描 workDir 兜底
                const candidates = fs.readdirSync(workDir).filter(function (f) { return f.toLowerCase().endsWith('.pdf'); });
                for (const cand of candidates) {
                    const candPath = path.join(workDir, cand);
                    if (fs.existsSync(candPath) && fs.statSync(candPath).size >= 1024) {
                        pdfPath = candPath;
                        break;
                    }
                }
            }
            if (!pdfPath) {
                throw new Error('无法转换该文档：未生成有效 PDF（' + path.basename(filePath) + '），支持 ' + OFFICE_SUPPORT_HINT);
            }
            return pdfPath;
        } catch (err) {
            try { fs.rmSync(workDir, { recursive: true, force: true }); } catch (e) {}
            throw err;
        } finally {
            this._releaseOfficeProfile(slot);
        }
    }

    async removeDarkBackground(inputBuffer) {
        const tempInput = path.join(this.tempDir, `black_in_${Date.now()}.jpg`);
        const tempProcessed = path.join(this.tempDir, `black_out_${Date.now()}.jpg`);
        
        try {
            fs.writeFileSync(tempInput, inputBuffer);
            const cmd = `convert "${tempInput}" -colorspace Gray ${this.AUTO_BLACK_CONFIG} "${tempProcessed}"`;
            await execPromise(cmd, this.TO.long);
            if (fs.existsSync(tempProcessed) && fs.statSync(tempProcessed).size > 1024) {
                return fs.readFileSync(tempProcessed);
            } else {
                throw new Error('处理失败');
            }
        } catch (error) {
            console.log(`[Print] 去黑底失败: ${error.message}`);
            const sharp = require('sharp');
            return await sharp(inputBuffer).threshold(180).toBuffer();
        } finally {
            try { if (fs.existsSync(tempInput)) fs.unlinkSync(tempInput); } catch(e) {}
            try { if (fs.existsSync(tempProcessed)) fs.unlinkSync(tempProcessed); } catch(e) {}
        }
    }

    // 任意图片 buffer → 稳妥 JPEG（先 sharp，失败再用 ImageMagick 强行转；均失败回退原图）
    async _imageBufferToJpg(buffer) {
        try {
            const sharp = require('sharp');
            const out = await sharp(buffer).jpeg().toBuffer();
            if (out && out.length > 0) return out;
        } catch (e) { /* sharp 不支持该格式，尝试 ImageMagick */ }
        try {
            const ti = path.join(this.tempDir, `ij_${Date.now()}_${Math.random().toString(16).slice(2, 6)}.in`);
            const tj = path.join(this.tempDir, `ij_${Date.now()}_${Math.random().toString(16).slice(2, 6)}.jpg`);
            fs.writeFileSync(ti, buffer);
            await execPromise(`convert "${ti}" -background white -flatten "${tj}"`, this.TO.proc);
            const out = fs.readFileSync(tj);
            try { fs.unlinkSync(ti); } catch (e) {}
            try { fs.unlinkSync(tj); } catch (e) {}
            if (out && out.length > 0) return out;
        } catch (e) { /* 转码失败，回退原图 */ }
        return buffer;
    }

    // 图片 buffer → PDF（img2pdf 直转；失败用 _imageBufferToJpg 转码后重试）
    async _pdfFromImageBuffer(buffer) {
        const tempInput = path.join(this.tempDir, `pi_${Date.now()}_${Math.random().toString(16).slice(2, 6)}`);
        const tempPdf = path.join(this.tempDir, `pi_${Date.now()}_${Math.random().toString(16).slice(2, 6)}.pdf`);
        try {
            fs.writeFileSync(tempInput, buffer);
            try {
                await execPromise(`img2pdf --rotation=ifvalid "${tempInput}" -o "${tempPdf}"`, this.TO.proc);
            } catch (e1) {
                const jpg = await this._imageBufferToJpg(buffer);
                fs.writeFileSync(tempInput, jpg);
                try {
                    await execPromise(`img2pdf --rotation=ifvalid "${tempInput}" -o "${tempPdf}"`, this.TO.proc);
                } catch (e2) {
                    throw new Error('图片转PDF失败（格式可能不被支持，建议先转成JPG）：' + (e2 && e2.message ? e2.message : e2));
                }
            }
            if (!fs.existsSync(tempPdf) || fs.statSync(tempPdf).size < 1024) {
                throw new Error('图片转PDF失败（格式可能不被支持，建议先转成JPG）');
            }
            return fs.readFileSync(tempPdf);
        } finally {
            try { if (fs.existsSync(tempInput)) fs.unlinkSync(tempInput); } catch (e) {}
            try { if (fs.existsSync(tempPdf)) fs.unlinkSync(tempPdf); } catch (e) {}
        }
    }

    async doPrintImage(buffer) {
        const pdf = await this._pdfFromImageBuffer(buffer);
        const tempPdf = path.join(this.tempDir, `print_${Date.now()}.pdf`);
        try {
            fs.writeFileSync(tempPdf, pdf);
            if (!fs.existsSync(tempPdf) || fs.statSync(tempPdf).size < 1024) {
                throw new Error('PDF无效');
            }
            await execPromise(`lp -d "${this.printerName}" -o fit-to-page -o position=center "${tempPdf}"`, this.TO.proc);
            return true;
        } finally {
            try { if (fs.existsSync(tempPdf)) fs.unlinkSync(tempPdf); } catch (e) {}
        }
    }

    async doPrintPdf(buffer) {
        const tempPdf = path.join(this.tempDir, `pdf_${Date.now()}.pdf`);
        try {
            fs.writeFileSync(tempPdf, buffer);
            if (!fs.existsSync(tempPdf) || fs.statSync(tempPdf).size < 1024) {
                throw new Error('PDF无效');
            }
            await execPromise(`lp -d "${this.printerName}" -o fit-to-page -o position=center "${tempPdf}"`, this.TO.proc);
            return true;
        } finally {
            try { if (fs.existsSync(tempPdf)) fs.unlinkSync(tempPdf); } catch(e) {}
        }
    }

    async doPrintDoubleSided(buf1, buf2) {
        const j1 = await this._imageBufferToJpg(buf1);
        const j2 = await this._imageBufferToJpg(buf2);
        const f1 = path.join(this.tempDir, `d1_${Date.now()}_${Math.random().toString(16).slice(2, 6)}.jpg`);
        const f2 = path.join(this.tempDir, `d2_${Date.now()}_${Math.random().toString(16).slice(2, 6)}.jpg`);
        const tempPdf = path.join(this.tempDir, `duplex_${Date.now()}_${Math.random().toString(16).slice(2, 6)}.pdf`);
        try {
            fs.writeFileSync(f1, j1);
            fs.writeFileSync(f2, j2);
            await execPromise(`img2pdf "${f1}" "${f2}" --rotation=ifvalid -o "${tempPdf}"`, this.TO.proc);
            await execPromise(`lp -d "${this.printerName}" -o fit-to-page -o position=center -o sides=two-sided-long-edge "${tempPdf}"`, this.TO.proc);
            return true;
        } finally {
            [f1, f2, tempPdf].forEach(f => { try { if (f && fs.existsSync(f)) fs.unlinkSync(f); } catch(e) {} });
        }
    }

    // 统一打印入口
    async printImage(groupId, buffer, session, isDemo) {
        if (isDemo) {
            await this.sendAndAutoRecall(groupId, '[演示] 模拟打印', session.msgIds, 10000);
            return true;
        }
        return await this.doPrintImage(buffer);
    }

    async printPdf(groupId, buffer, session, isDemo) {
        if (isDemo) {
            await this.sendAndAutoRecall(groupId, '[演示] 模拟打印PDF', session.msgIds, 10000);
            return true;
        }
        return await this.doPrintPdf(buffer);
    }

    async printDoubleSided(groupId, buf1, buf2, session, isDemo) {
        if (isDemo) {
            await this.sendAndAutoRecall(groupId, '[演示] 模拟双面打印', session.msgIds, 10000);
            return true;
        }
        return await this.doPrintDoubleSided(buf1, buf2);
    }

    // 双面模式收到 PDF：整份 PDF 由 CUPS 自动双面（一张 PDF = 一次双面任务，内部多页由 CUPS 处理）
    async doPrintPdfDoubleSided(buffer) {
        const tempPdf = path.join(this.tempDir, `pdf_duplex_${Date.now()}.pdf`);
        try {
            fs.writeFileSync(tempPdf, buffer);
            if (!fs.existsSync(tempPdf) || fs.statSync(tempPdf).size < 1024) {
                throw new Error('PDF无效');
            }
            await execPromise(`lp -d "${this.printerName}" -o fit-to-page -o position=center -o sides=two-sided-long-edge "${tempPdf}"`, this.TO.proc);
            return true;
        } finally {
            try { if (fs.existsSync(tempPdf)) fs.unlinkSync(tempPdf); } catch(e) {}
        }
    }

    async printPdfDoubleSided(groupId, buffer, session, isDemo) {
        if (isDemo) {
            await this.sendAndAutoRecall(groupId, '[演示] 模拟双面打印PDF', session.msgIds, 10000);
            return true;
        }
        return await this.doPrintPdfDoubleSided(buffer);
    }

    getLayouts(n) {
        const results = [];
        const seen = new Set();
        const add = (cols, rows) => {
            if (cols > this.MAX_COLS || rows > this.MAX_ROWS) return;
            const key = `${cols}x${rows}`;
            if (!seen.has(key) && cols * rows >= n) {
                seen.add(key);
                results.push({ rows, cols });
            }
        };
        if (n <= 1) return [{ rows: 1, cols: 1 }];
        if (n === 2) { add(1, 2); add(2, 1); }
        else if (n <= 4) { add(2, 2); }
        else {
            for (let cols = 1; cols <= this.MAX_COLS; cols++) {
                for (let rows = 1; rows <= this.MAX_ROWS; rows++) {
                    if (cols * rows >= n && cols * rows <= n + 3) add(cols, rows);
                }
            }
            if (results.length === 0) {
                const sq = Math.ceil(Math.sqrt(n));
                add(Math.min(sq, this.MAX_COLS), Math.min(sq, this.MAX_ROWS));
            }
        }
        results.sort((a, b) => (a.rows * a.cols) - (b.rows * b.cols));
        return results;
    }

    async stitchImages(buffers, rows, cols, startIdx, count) {
        const workDir = path.join(this.tempDir, `stitch_${Date.now()}`);
        fs.mkdirSync(workDir, { recursive: true });
        try {
            const totalCells = rows * cols;
            const halfGap = Math.floor(this.GAP / 2);
            const cellW = Math.floor((this.A4_W - this.GAP * 2 - this.GAP * (cols - 1)) / cols);
            const cellH = Math.floor((this.A4_H - this.GAP * 2 - this.GAP * (rows - 1)) / rows);
            
            const cellFiles = [];
            for (let i = 0; i < count; i++) {
                const tempImg = path.join(workDir, `in_${i}.jpg`);
                const outImg = path.join(workDir, `out_${i}.jpg`);
                fs.writeFileSync(tempImg, buffers[startIdx + i]);
                await execPromise(
                    `convert "${tempImg}" -fuzz 5% -trim +repage -resize ${cellW}x${cellH} -background white -gravity center -extent ${cellW}x${cellH} -bordercolor white -border ${halfGap}x${halfGap} "${outImg}"`,
                    30000
                );
                cellFiles.push(outImg);
                try { fs.unlinkSync(tempImg); } catch(e) {}
            }
            while (cellFiles.length < totalCells) {
                const blank = path.join(workDir, `blank_${cellFiles.length}.jpg`);
                await execPromise(`convert -size ${cellW}x${cellH} xc:white -bordercolor white -border ${halfGap}x${halfGap} "${blank}"`, this.TO.net);
                cellFiles.push(blank);
            }
            
            const rowFiles = [];
            for (let r = 0; r < rows; r++) {
                const rowFile = path.join(workDir, `row_${r}.jpg`);
                await execPromise(`convert ${cellFiles.slice(r*cols, (r+1)*cols).map(f=>`"${f}"`).join(' ')} +append "${rowFile}"`, 30000);
                rowFiles.push(rowFile);
            }
            
            const gridFile = path.join(workDir, 'grid.jpg');
            await execPromise(`convert ${rowFiles.map(f=>`"${f}"`).join(' ')} -append "${gridFile}"`, this.TO.proc);
            
            const infoResult = await execPromise(`identify -format "%w %h" "${gridFile}"`, 10000);
            const infoStr = infoResult.stdout ? infoResult.stdout.trim() : infoResult.toString().trim();
            const [gridW, gridH] = infoStr.split(' ').map(Number);
            const offsetX = Math.floor((this.A4_W - gridW) / 2);
            const offsetY = Math.floor((this.A4_H - gridH) / 2);
            
            const outFile = path.join(workDir, 'output.jpg');
            await execPromise(`convert -size ${this.A4_W}x${this.A4_H} xc:white "${gridFile}" -geometry +${offsetX}+${offsetY} -composite "${outFile}"`, this.TO.proc);
            return fs.readFileSync(outFile);
        } finally {
            try { fs.rmSync(workDir, { recursive: true, force: true }); } catch(e) {}
        }
    }



    // ==========================================
    // 菜单命令
    // ==========================================

    async showMenu(context) {
        const isSuper = this.isSuperAdmin(context.userId);
        const isDev = this.isDeveloper(context.userId);
        const isDemo = this.isDemoMode(context.groupId);
        const recallStatus = this.recallEnabled ? '开启' : '关闭';
        
        let menu = '[打印菜单]\n';
        menu += '-------------------\n';
        menu += '[打印命令]\n';
        menu += '  直接发图/PDF  极速打印\n';
        menu += '  office 文档  支持 Word/Excel/PPT 直接打印\n';
        menu += '  去黑           去黑底打印\n';
        menu += '  双面           双面打印\n';
        menu += '  a3             A3切割（自动检测中缝）\n';
        menu += '  拼图           多图拼图\n';
        menu += '  队列           查看打印队列\n';
        menu += '  #打印状态     打印机实时状态\n';
        menu += '  取消           退出当前模式\n';
        menu += '-------------------\n';
        menu += '[系统状态]\n';
        menu += '  撤回功能: ' + recallStatus + '\n';
        if (isDemo) {
            menu += '  演示模式: 已开启\n';
        }
        menu += '[WEB 打印]\n';
        menu += '  Web 打印     打开管理台 → Web 打印\n';
        menu += '               上传图片/PDF/文档 提交打印（可离线排队）\n';
        menu += '-------------------\n';
        menu += '[管理命令]\n';
        menu += '  菜单           显示此菜单\n';
        menu += '  开启撤回       开启自动撤回\n';
        menu += '  关闭撤回       关闭自动撤回\n';
        if (isSuper || isDev) {
            menu += '  监听此群       添加到白名单\n';
            menu += '  不监听此群     移出白名单\n';
            menu += '  开启演示       演示模式\n';
            menu += '  关闭演示       恢复打印\n';
            menu += '  切换框架       切换sea1 / 切换sea2\n';
            menu += '                切换主框架 / 切换副框架 / 切换框架\n';
            menu += '                 （仅管理员·Sea print 群）\n';
        }
        menu += '-------------------\n';
        menu += '[√] 发送「帮助」查看详细说明';
        
        await context.reply(menu);
    }

    // ==========================================
    // 消息处理
    // ==========================================

    async onMessage(context) {
        // ===== QA内部测试入口 =====
        if (context.msg === 'QAtest') {
            console.log('[QAtest] 测试触发成功!');
            try {
                await context.reply('[QAtest] 插件正常运行! 时间: ' + new Date().toISOString());
            } catch(e) { console.log('[QAtest] reply错误: ' + e.message); }
            return true;
        }
        // ===== 测试入口结束 =====

        const userId = context.userId;
        const groupId = context.groupId;
        const msg = context.msg.trim();

        console.log(`[Print] 收到: ${msg.substring(0, 30)}`);

        if (!groupId) {
            return false;
        }

        const userMsgId = this.getUserMessageId(context);
        if (userMsgId) {
            this.recordUserMessage(userId, userMsgId);
        }

        let session = this.sessions.get(userId);
        if (!session) {
            session = { msgIds: [], lastActive: Date.now() };
            this.sessions.set(userId, session);
        }
        session.lastActive = Date.now();

        const isSuper = this.isSuperAdmin(userId);
        const isDev = this.isDeveloper(userId);
        const isDemo = this.isDemoMode(groupId);
        const isAllowed = this.isGroupAllowed(groupId);

        const ctx = { msg, userId, groupId, session, isSuper, isDev, isDemo, isAllowed, context, plugin: this };

        // Layer 0a: 全局命令（不受群组白名单限制）
        // (ctx 已含 context 字段供 handler 使用)
        for (const cmd of this.commands.global) {
            if (cmd.match(ctx)) {
                if (!this._authorize(ctx, cmd.perm)) { await ctx.context.reply('[×] 权限不足'); return true; }
                return await cmd.handle(ctx);
            }
        }

        // 群组白名单检查（非白名单群仅能使用全局命令）
        if (!isAllowed) {
            return false;
        }

        // Layer 0b: 白名单内全局命令（队列）
        for (const cmd of this.commands.allowed) {
            if (cmd.match(ctx)) {
                if (!this._authorize(ctx, cmd.perm)) { await ctx.context.reply('[×] 权限不足'); return true; }
                return await cmd.handle(ctx);
            }
        }

        // [FIX-2026-08-08] 模式入口词优先：会话卡在旧模式时，发模式入口词直接重新进入（先清旧 session.state）
        const _modeEntryWords = ['双面', '双面打印', '去黑', '去黑底', 'a3', 'A3', '拼图'];
        if (session.state && _modeEntryWords.includes(msg)) {
          if (session.timer) { try { clearTimeout(session.timer); } catch (e) {} session.timer = null; }
          session.state = null;
        }

        // [打印门禁] 实际提交打印内容（图/文件）或进入打印模式前，先校验服务端试用/会员资格
        const _printSubmit = msg.includes('[CQ:image') || msg.includes('[CQ:file');
        const _modeEntryHit = !session.state && this.commands.modeEntry.some((cmd) => {
            try { return cmd.match(ctx); } catch (e) { return false; }
        });
        if (_printSubmit || _modeEntryHit) {
            const gate = await this._checkPrintPerm(ctx);
            if (!gate.allow) {
                await this.sendGroupMessage(groupId, (gate.lines && gate.lines.join('\n')) || '[×] 暂无打印权限');
                return true;
            }
        }

        // Layer 2: 模式内分发
        if (session.state) {
            const disp = this.modeDispatchers[session.state];
            if (disp) {
                const handled = await disp(ctx);
                if (handled) return true;
            }
        }

        // Layer 1: 模式入口
        for (const cmd of this.commands.modeEntry) {
            if (cmd.match(ctx)) {
                if (!this._authorize(ctx, cmd.perm)) { await ctx.context.reply('[×] 权限不足'); return true; }
                return await cmd.handle(ctx);
            }
        }

        // Layer 3: 无模式极速打印
        if (!session.state && (msg.includes('[CQ:image') || msg.includes('[CQ:file'))) {
            return await this._instantPrint(ctx);
        }

        // 兜底：无关消息
        if (!session.state) {
            // 本插件无活跃会话且不认识该指令 → 放行给后续插件（如 vip 优先级90）处理，
            // 不再强行 return true 拦截，否则会"吞掉"会员/菜单外指令。
            return false;
        }
        // 有活跃会话但不匹配任何命令才提示
        await this.sendGroupMessage(groupId,
            '[!] 无活跃任务\n' +
            '发「去黑」「双面」「a3」「拼图」\n' +
            '直接发图极速打印\n' +
            '发「菜单」查看所有命令'
        );
        return true;
    }

    // ==========================================
    // 全局命令 handler
    // ==========================================
    async _cmdMenu(c) {
        await this.showMenu(c.context);
        return true;
    }

    async _cmdRecall(c) {
        this.recallEnabled = c.msg === '开启撤回';
        await this.saveConfig();
        await c.context.reply('[√] 已' + (this.recallEnabled ? '开启' : '关闭') + '撤回');
        return true;
    }

    async _cmdListen(c, add) {
        const { groupId } = c;
        if (add) {
            if (this.isGroupAllowed(groupId)) {
                await c.context.reply('[!] 已在监听列表');
            } else {
                await this.addGroup(groupId);
                await c.context.reply('[√] 已添加此群');
            }
        } else {
            if (!this.isGroupAllowed(groupId)) {
                await c.context.reply('[!] 不在监听列表');
            } else {
                await this.removeGroup(groupId);
                await c.context.reply('[√] 已移除监听');
            }
        }
        return true;
    }

    async _cmdDemo(c, on) {
        const { groupId } = c;
        if (on) {
            if (this.isDemoMode(groupId)) {
                await c.context.reply('[!] 已是演示模式');
            } else {
                await this.enableDemo(groupId);
                await c.context.reply('[√] 已开启演示');
            }
        } else {
            if (!this.isDemoMode(groupId)) {
                await c.context.reply('[!] 不是演示模式');
            } else {
                await this.disableDemo(groupId);
                await c.context.reply('[√] 已关闭演示');
            }
        }
        return true;
    }

    async _cmdHelp(c) {
        let help = '[智能打印]\n';
        help += '-------------------\n';
        help += '直接发图/PDF  极速打印\n';
        help += 'office 文档  支持 Word/Excel/PPT 直接打印\n';
        help += '去黑           去黑底打印\n';
        help += '双面           双面打印\n';
        help += 'a3             A3切割\n';
        help += '拼图           多图拼图\n';
        help += '队列           查看打印队列\n';
        help += '#打印状态      打印机实时状态\n';
        help += '菜单           显示所有命令\n';
        help += '取消           退出当前模式\n';
        help += '-------------------\n';
        help += '[会员服务]\n';
        help += '开通会员：选择时长并开通会员\n';
        help += '我的会员：查看当前会员状态与到期时间\n';
        if (c.isSuper || c.isDev) {
            help += '付款记录：查看历史订单与支付状态\n';
            help += '推送查询：查询激活/发码推送状态\n';
        }
        help += '-------------------\n';
        help += '[WEB 打印]\n';
        help += 'Web 打印：打开管理台 → Web 打印，上传图片/PDF/文档 提交打印（可离线排队）\n';
        help += '-------------------\n';
        help += '[管理命令]\n';
        help += '开启撤回/关闭撤回  控制自动撤回\n';
        if (c.isSuper || c.isDev) {
            help += '监听此群/不监听此群  群组白名单\n';
            help += '开启演示/关闭演示  演示模式\n';
            help += '切换sea1/切换sea2/切换主框架/切换副框架/切换框架  （仅管理员·Sea print 群）\n';
        }
        await c.context.reply(help);
        return true;
    }

    async _cmdCancel(c) {
        const { groupId, userId, session } = c;
        this.queueSession = null;
        // 双面模式可能挂有 3 秒防抖计时器，删除会话前先清理，避免会话删除后仍触发
        if (session && session.timer) { clearTimeout(session.timer); session.timer = null; }
        // 快照待撤回消息 id 后立即清空会话，避免多条消息逐条撤回（数百 ms/条）累计延迟
        const msgIds = (session && session.msgIds && session.msgIds.length > 0) ? session.msgIds.slice() : [];
        if (session) session.msgIds = [];
        this.sessions.delete(userId);
        // 先秒回「已取消」，撤回逻辑放到回复之后异步执行（逐条撤 + 撤用户消息），
        // 保证群内回复不被撤回耗时拖慢数秒；try/catch 吞错，撤回失败不影响主流程。
        await this.sendAndAutoRecall(groupId, '[OK] 已取消', [], 10000);
        setImmediate(() => {
            (async () => {
                try {
                    for (const id of msgIds) {
                        await this.recallMessage(id);
                    }
                } catch (e) { /* 撤回失败不影响主流程 */ }
                try {
                    await this.recallUserMessages(userId);
                } catch (e) { /* 撤回失败不影响主流程 */ }
            })();
        });
        return true;
    }

    async _cmdStatus(c) {
        const { groupId, session, isDemo } = c;
        try {
            const result = await execPromise(`lpstat -p -d 2>/dev/null`);
            const out = result.stdout ? result.stdout : result.toString();
            let status = '[打印机状态]\n';
            status += '-------------------\n';
            status += '当前: ' + this.printerName + '\n';
            status += out.split('\n').filter(l => l.trim()).slice(0, 3).join('\n');
            if (isDemo) {
                status += '\n[演示模式]';
            }
            await this.sendAndAutoRecall(groupId, status, session.msgIds, 10000);
        } catch (e) {
            await this.sendAndAutoRecall(groupId, '错误: ' + e.message, session.msgIds, 10000);
        }
        return true;
    }

    async _cmdQueue(c) {
        const { groupId, userId, session } = c;
        const queueOut = await this.getPrintQueue(this.printerName);
        const queueStr = this.formatQueuePretty(queueOut);
        if (!queueStr) {
            await this.sendAndAutoRecall(groupId, '打印队列为空', session.msgIds, 10000);
            return true;
        }
        this.queueSession = { userId: userId, groupId: groupId, msgIds: session.msgIds };
        await this.sendAndAutoRecall(groupId, queueStr, session.msgIds, 60000);
        return true;
    }

    async _cmdQueueQuit(c) {
        const { groupId, session } = c;
        this.queueSession = null;
        await this.sendAndAutoRecall(groupId, '已退出队列', session.msgIds, 5000);
        return true;
    }

    async _cmdQueuePick(c) {
        const { groupId, session, msg } = c;
        const num = parseInt(msg);
        if (num === 0) {
            const cancelled = await this.cancelAllJobs(this.printerName);
            await this.sendAndAutoRecall(groupId, '已取消 ' + cancelled + ' 个任务', session.msgIds, 10000);
        } else {
            const jobId = `${this.printerName}-${num}`;
            const success = await this.cancelPrintJob(jobId);
            if (success) {
                await this.sendAndAutoRecall(groupId, '已取消任务 ' + num, session.msgIds, 10000);
            } else {
                await this.sendAndAutoRecall(groupId, '任务 ' + num + ' 不存在', session.msgIds, 10000);
            }
        }
        const queueOut = await this.getPrintQueue(this.printerName);
        const queueStr = this.formatQueuePretty(queueOut);
        if (queueStr) {
            await this.sendAndAutoRecall(groupId, queueStr, session.msgIds, 60000);
        } else {
            await this.sendAndAutoRecall(groupId, '打印队列已清空', session.msgIds, 10000);
            this.queueSession = null;
        }
        return true;
    }

    // ==========================================
    // 模式入口 handler
    // ==========================================
    _newSession(userId, state, extra) {
        const s = Object.assign({ state: state, msgIds: [], lastActive: Date.now() }, extra || {});
        this.sessions.set(userId, s);
        return s;
    }

    async _enterDuplex(c) {
        const { groupId, userId, isDemo } = c;
        // 新双面模式：批量静默收集 + 3秒防抖 + 按序两两配对双面打印
        const s = this._newSession(userId, 'duplex', {
            queue: [],            // 当前批次收集的图片 buffer
            leftoverSingle: null, // 奇数批次剩的单张（未打印）
            printing: false,      // 是否有批次正在打印
            lastPairs: null,      // 上一已打印批次的配对面 buffer 数组（用于回复1重印，不含剩单张）
            timer: null           // 3秒防抖计时器句柄
        });
        let tip = '[双面打印(批量)]\n';
        tip += '--------\n';
        tip += '发图静默收集，停3秒自动按序两两双面打印\n';
        tip += '打印中可继续发图(新批次)\n';
        tip += '打完回复 1 再打一份\n';
        tip += '剩单张发「完成」打印最后单张\n';
        tip += '发「取消」退出';
        if (isDemo) { tip += '\n[演示模式]'; }
        await this.sendAndAutoRecall(groupId, tip, s.msgIds, 10000);
        return true;
    }

    async _enterBlack(c) {
        const { groupId, userId, isDemo } = c;
        const s = this._newSession(userId, 'black', {});
        let tip = '[去黑底]\n';
        tip += '-------------------\n';
        tip += '发图自动去黑底\n';
        tip += '发预览后打印\n';
        tip += '发「取消」退出';
        if (isDemo) { tip += '\n[演示模式]'; }
        await this.sendAndAutoRecall(groupId, tip, s.msgIds, 10000);
        return true;
    }

    async _enterA3(c) {
        const { groupId, userId, isDemo } = c;
        const s = this._newSession(userId, 'a3', { cutJobs: [] });
        let tip = '[A3切割]\n';
        tip += '-------------------\n';
        tip += '发A3图片或PDF（可多张）\n';
        tip += '自动检测中缝切割\n';
        tip += '发「完成」选打印方式\n';
        tip += '发「取消」退出';
        if (isDemo) { tip += '\n[演示模式]'; }
        await this.sendAndAutoRecall(groupId, tip, s.msgIds, 10000);
        return true;
    }

    async _enterStitch(c) {
        const { groupId, userId, isDemo } = c;
        const s = this._newSession(userId, 'stitch', { images: [] });
        let tip = '[拼图]\n';
        tip += '-------------------\n';
        tip += '发图，发「完成」开始拼\n';
        tip += '发「取消」退出';
        if (isDemo) { tip += '\n[演示模式]'; }
        await this.sendAndAutoRecall(groupId, tip, s.msgIds, 10000);
        return true;
    }

    // ==========================================
    // 模式内分发
    // ==========================================
    async _dispatchDuplex(c) {
        const { msg, groupId, userId, session, isDemo } = c;
        // 回复 1：重印上一已打印批次的配对面（不含剩单张）
        if (msg === '1') {
            if (!session.lastPairs || session.lastPairs.length === 0) {
                await this.sendAndAutoRecall(groupId, '还没有可重印的批次', session.msgIds, 10000);
                return true;
            }
            const pairs = session.lastPairs;
            const n = pairs.length;
            const groups = Math.floor(n / 2);
            await this.sendAndAutoRecall(groupId, '重印 ' + groups + ' 组...', session.msgIds, 5000);
            try {
                for (let i = 0; i < n; i += 2) {
                    await this.printDoubleSided(groupId, pairs[i], pairs[i + 1], session, isDemo);
                }
                // 若上一批次为奇数，剩单张补打一张单面
                if (n % 2 === 1 && session.leftoverSingle) {
                    await this.sendAndAutoRecall(groupId, '重印剩单张...', session.msgIds, 5000);
                    await this.printImage(groupId, session.leftoverSingle, session, isDemo);
                }
                await this.sendAndAutoRecall(groupId, '✅ 重印完成', session.msgIds, 10000);
            } catch (error) {
                await this.sendAndAutoRecall(groupId, '错误: ' + error.message, session.msgIds, 10000);
                console.error(`[Print]`, error);
            }
            return true;
        }
        // 完成：清定时器 → flush 余图 → 打印剩单张 → 删会话
        if (msg === '完成') {
            if (session.timer) { clearTimeout(session.timer); session.timer = null; }
            // 若上一批次仍在打印，先等它结束，避免竞态丢图
            let _guard = 0;
            while (session.printing && _guard < 50) {
                await new Promise(r => setTimeout(r, 100));
                _guard++;
            }
            if (session.queue && session.queue.length > 0) {
                await this._flushDuplex(groupId, userId, session, isDemo, c.context);
            }
            if (session.leftoverSingle) {
                await this.sendAndAutoRecall(groupId, '剩单张单面打印...', session.msgIds, 5000);
                await this.printImage(groupId, session.leftoverSingle, session, isDemo);
                session.leftoverSingle = null;
                await this.sendAndAutoRecall(groupId, '单面打印完成', session.msgIds, 10000);
            }
            if (session.msgIds && session.msgIds.length > 0) {
                for (const id of session.msgIds) { await this.recallMessage(id); }
                session.msgIds = [];
            }
            this.sessions.delete(userId);
            await this.sendAndAutoRecall(groupId, '已完成', session.msgIds, 10000);
            return true;
        }
        // 收图：静默入队 + 重置 3 秒防抖计时器
        if (msg.includes('[CQ:image') || msg.includes('[CQ:file')) {
            try {
                const result = await this.downloadFile(msg);
                session.lastActive = Date.now();
                // PDF：作为独立双面任务直接走 CUPS 双面打印（PDF 内部多页由 CUPS 自动双面），
                // 不入图片配对队列（修复：PDF 被当 1 张图导致 "0组/剩1张单页" 的错误处理）
                if (result.isPdf) {
                    await this.sendAndAutoRecall(groupId, 'PDF双面打印...', session.msgIds, 5000);
                    await this.printPdfDoubleSided(groupId, result.buffer, session, isDemo);
                    await this.sendAndAutoRecall(groupId, '✅ PDF双面打印完成', session.msgIds, 10000);
                    return true;
                }
                // 上一批次剩单张存在：将新图与剩单张配对成双面组（续接上一批）
                if (session.leftoverSingle) {
                    const front = session.leftoverSingle;
                    session.leftoverSingle = null;
                    await this.sendAndAutoRecall(groupId, '双面打印(续接上批剩单张)...', session.msgIds, 5000);
                    await this.printDoubleSided(groupId, front, result.buffer, session, isDemo);
                    await this.sendAndAutoRecall(groupId, '✅ 续接完成', session.msgIds, 10000);
                    return true;
                }
                // 图片按 buffer 入队（仅图片走配对；PDF 已在上面分流）
                session.queue.push(result.buffer);
                this._scheduleDuplexFlush(session, c);
            } catch (error) {
                await this.sendAndAutoRecall(groupId, '错误: ' + error.message, session.msgIds, 10000);
                console.error(`[Print]`, error);
            }
            return true;
        }
        return false;
    }

    // 3 秒防抖：每次收图重置计时器，连续 3 秒无新图触发本批 flush
    _scheduleDuplexFlush(session, c) {
        if (session.timer) { clearTimeout(session.timer); }
        session.timer = setTimeout(() => {
            session.timer = null;
            this._flushDuplex(c.groupId, c.userId, session, c.isDemo, c.context);
        }, 3000);
    }

    // 批量 flush：快照队列 → 按序两两配对双面打印 → 奇数剩单张
    async _flushDuplex(groupId, userId, session, isDemo, context) {
        // 会话可能已被删除（如取消），直接退出避免报错
        if (!this.sessions.has(userId)) return;
        // 若正在打印其它批次，延迟重试，避免并发 flush 同一队列
        if (session.printing) {
            setTimeout(() => {
                this._flushDuplex(groupId, userId, session, isDemo, context);
            }, 500);
            return;
        }
        // 同步快照本批，使打印期间到达的新图进入新队列
        const batch = session.queue;
        session.queue = [];
        if (!batch || batch.length === 0) return;
        session.printing = true;
        try {
            const n = batch.length;
            const groups = Math.floor(n / 2);
            await this.sendAndAutoRecall(groupId, '收到 ' + n + ' 张，双面打印中(' + groups + '组)...', session.msgIds, 5000);
            for (let i = 0; i < n - 1; i += 2) {
                await this.printDoubleSided(groupId, batch[i], batch[i + 1], session, isDemo);
            }
            // 记录已打印批次配对面，供回复 1 重印（不含剩单张）
            session.lastPairs = (n % 2 === 1) ? batch.slice(0, n - 1) : batch.slice();
            if (n % 2 === 1) {
                // 奇数张：最后 1 张作为剩单张保留，等待「完成」单面打印或继续配对
                session.leftoverSingle = batch[n - 1];
                await this.sendAndAutoRecall(groupId,
                    '✅ 双面打印完成 共' + groups + '组\n剩 1 张单页：发「完成」打印最后单张，或继续发图(将与前一张配对)；本批已印，回复 1 可重印',
                    session.msgIds, 10000);
            } else {
                session.leftoverSingle = null;
                await this.sendAndAutoRecall(groupId,
                    '✅ 双面打印完成 共' + groups + '组 / 再打一份回复 1',
                    session.msgIds, 10000);
            }
        } catch (error) {
            await this.sendAndAutoRecall(groupId, '错误: ' + error.message, session.msgIds, 10000);
            console.error(`[Print]`, error);
        } finally {
            session.printing = false;
        }
    }

    async _dispatchBlack(c) {
        const { msg, groupId, userId, session, isDemo } = c;
        if (!(msg.includes('[CQ:image') || msg.includes('[CQ:file'))) return false;
        try {
            const result = await this.downloadFile(msg);
            // PDF：复用 A3 模式的 PDF 处理方法（逐页渲染），每页走去黑流程，不去图片分支
            if (result.isPdf) {
                await this.sendAndAutoRecall(groupId, 'PDF逐页渲染...', session.msgIds, 5000);
                const pdfImgs = await this.renderPdfToImages(result.buffer);
                if (!pdfImgs || pdfImgs.length === 0) {
                    await this.sendAndAutoRecall(groupId, 'PDF无页面', session.msgIds, 10000);
                    return true;
                }
                for (let i = 0; i < pdfImgs.length; i++) {
                    const pagePath = pdfImgs[i];
                    const pageBuf = fs.readFileSync(pagePath);
                    await this.sendAndAutoRecall(groupId, '去黑处理(第 ' + (i + 1) + '/' + pdfImgs.length + ' 页)...', session.msgIds, 5000);
                    const processed = await this.removeDarkBackground(pageBuf);
                    await this.sendPreview(groupId, processed, '[预览]');
                    await this.sendAndAutoRecall(groupId, '打印...', session.msgIds, 5000);
                    await this.printImage(groupId, processed, session, isDemo);
                    try { if (fs.existsSync(pagePath)) fs.unlinkSync(pagePath); } catch(e) {}
                }
                await this.recallUserMessages(userId);
                await this.sendAndAutoRecall(groupId, '打印完成', session.msgIds, 10000);
                this._afterPrintDone(c);
                return true;
            }
            // 图片：原有去黑流程
            await this.sendAndAutoRecall(groupId, '去黑处理...', session.msgIds, 5000);
            const processed = await this.removeDarkBackground(result.buffer);
            await this.sendPreview(groupId, processed, '[预览]');
            await this.recallUserMessages(userId);
            await this.sendAndAutoRecall(groupId, '打印...', session.msgIds, 5000);
            await this.printImage(groupId, processed, session, isDemo);
            await this.sendAndAutoRecall(groupId, '打印完成', session.msgIds, 10000);
            this._afterPrintDone(c);
        } catch (error) {
            await this.sendAndAutoRecall(groupId, '错误: ' + error.message, session.msgIds, 10000);
            console.error(`[Print]`, error);
        }
        return true;
    }

    async _dispatchA3(c) {
        const { msg, groupId, userId, session, isDemo } = c;
        // 完成 / 退出 / 取消 统一收尾
        if (msg === '完成' || msg === '退出' || msg === '取消') {
            if (msg === '完成') {
                if (session.subState === this.SUB.CUTS) {
                    await this.sendAndAutoRecall(groupId, '请先回复切割份数', session.msgIds, 10000);
                    return true;
                }
                if (!session.cutJobs || session.cutJobs.length === 0) {
                    await this.sendAndAutoRecall(groupId, '还没有图', session.msgIds, 10000);
                    return true;
                }
                session.subState = this.SUB.PRINT;
                await this.sendAndAutoRecall(groupId,
                    '共 ' + session.cutJobs.reduce((a, j) => a + j.buffers.length, 0) + ' 张，选打印方式:\n' +
                    '1. 单面打印\n' +
                    '2. 双面打印\n' +
                    '取消退出',
                    session.msgIds, 15000
                );
            } else {
                this.sessions.delete(userId);
                await this.sendAndAutoRecall(groupId, '已退出A3', session.msgIds, 10000);
            }
            return true;
        }
        // 接收图片/PDF（收图阶段：subState 为空）
        if ((msg.includes('[CQ:image') || msg.includes('[CQ:file')) && !session.subState) {
            try {
                const result = await this.downloadFile(msg);
                let sheets = [];
                if (result.isPdf) {
                    await this.sendAndAutoRecall(groupId, 'PDF逐页渲染...', session.msgIds, 5000);
                    const pdfImgs = await this.renderPdfToImages(result.buffer);
                    if (!pdfImgs || pdfImgs.length === 0) {
                        await this.sendAndAutoRecall(groupId, 'PDF无页面', session.msgIds, 10000);
                        return true;
                    }
                    for (const p of pdfImgs) sheets.push({ imagePath: p, preview: false });
                } else {
                    const imagePath = path.join(this.tempDir, `a3_${Date.now()}.jpg`);
                    fs.writeFileSync(imagePath, result.buffer);
                    sheets.push({ imagePath: imagePath, preview: true });
                }
                for (let si = 0; si < sheets.length; si++) {
                    const sh = sheets[si];
                    const sizeOut = await execPromise(`identify -format "%w %h" "${sh.imagePath}"`, this.TO.mid);
                    const sizeStr = sizeOut.stdout ? sizeOut.stdout.trim() : sizeOut.toString().trim();
                    const [w, h] = sizeStr.split(/\s+/).map(Number);
                    const dir = w > h ? 'vertical' : 'horizontal';
                    const dirLabel = dir === 'vertical' ? '竖版(左右切)' : '横版(上下切)';
                    if (sheets.length > 1) {
                        await this.sendAndAutoRecall(groupId, '第 ' + (si + 1) + '/' + sheets.length + ' 页: ' + dirLabel + ' | ' + w + 'x' + h, session.msgIds, 5000);
                    } else {
                        await this.sendAndAutoRecall(groupId, '方向: ' + dirLabel + ' | ' + w + 'x' + h, session.msgIds, 5000);
                    }
                    const detectResult = await this.detectParts(sh.imagePath, w, h);
                    const parts = detectResult.parts;
                    const positions = detectResult.positions;
                    if (parts === 0) {
                        session.subState = this.SUB.CUTS;
                        session.imagePath = sh.imagePath;
                        session.direction = dir;
                        await this.sendAndAutoRecall(groupId, '未检测到中缝\n请回复切割份数 (2-9)', session.msgIds, 10000);
                        return true;
                    }
                    await this.sendAndAutoRecall(groupId, '检测到 ' + parts + ' 开，中缝位置: ' + positions.map(p => (p * 100).toFixed(0) + '%').join(', '), session.msgIds, 5000);
                    await this.sendAndAutoRecall(groupId, '正在切割...', session.msgIds, 5000);
                    const cutFiles = await this.splitImageByPosition(sh.imagePath, dir, positions);
                    const buffers = [];
                    for (const f of cutFiles) {
                        const buf = fs.readFileSync(f);
                        buffers.push(buf);
                        if (sh.preview) {
                            const preview = path.join(this.tempDir, `prev_${Date.now()}.jpg`);
                            await execPromise(`convert "${f}" "${preview}"`, this.TO.net);
                            const previewBuf = fs.readFileSync(preview);
                            await this.sendPreview(groupId, previewBuf, '预览');
                            try { fs.unlinkSync(preview); } catch(e) {}
                        }
                        try { fs.unlinkSync(f); } catch(e) {}
                    }
                    try { if (fs.existsSync(sh.imagePath)) fs.unlinkSync(sh.imagePath); } catch(e) {}
                    if (!session.cutJobs) session.cutJobs = [];
                    session.cutJobs.push({ buffers: buffers });
                    if (sheets.length > 1) {
                        const acc = session.cutJobs.reduce((a, j) => a + j.buffers.length, 0);
                        await this.sendAndAutoRecall(groupId, '第 ' + (si + 1) + ' 页已切 ' + buffers.length + ' 张（累计 ' + acc + ' 张）', session.msgIds, 8000);
                    }
                }
                if (sheets.length === 1) {
                    const total = session.cutJobs.reduce((a, j) => a + j.buffers.length, 0);
                    await this.sendAndAutoRecall(groupId,
                        '已切割 ' + total + ' 张（累计 ' + total + ' 张）\n继续发图，或发「完成」选打印方式',
                        session.msgIds, 8000);
                } else {
                    const total = session.cutJobs.reduce((a, j) => a + j.buffers.length, 0);
                    await this.sendAndAutoRecall(groupId,
                        '共 ' + sheets.length + ' 页已切完，累计 ' + total + ' 张\n继续发图，或发「完成」选打印方式',
                        session.msgIds, 8000);
                }
            } catch (error) {
                await this.sendAndAutoRecall(groupId, '错误: ' + error.message, session.msgIds, 10000);
                console.error(`[Print] A3错误:`, error);
            }
            return true;
        }
        // 手动切割份数（WAITING_CUTS）
        if (session.subState === this.SUB.CUTS && /^\d+$/.test(msg)) {
            const num = parseInt(msg);
            if (num < 2 || num > 9) {
                await this.sendAndAutoRecall(groupId, '请输入2-9', session.msgIds, 10000);
                return true;
            }
            try {
                const imgPath = session.imagePath;
                const dir = session.direction;
                await this.sendAndAutoRecall(groupId, '切割为 ' + num + ' 份...', session.msgIds, 5000);
                const positions = [];
                for (let i = 1; i < num; i++) positions.push(i / num);
                const cutFiles = await this.splitImageByPosition(imgPath, dir, positions);
                const buffers = [];
                for (const f of cutFiles) {
                    const buf = fs.readFileSync(f);
                    buffers.push(buf);
                    const preview = path.join(this.tempDir, `prev_${Date.now()}.jpg`);
                    await execPromise(`convert "${f}" "${preview}"`, this.TO.net);
                    const previewBuf = fs.readFileSync(preview);
                    await this.sendPreview(groupId, previewBuf, '预览');
                    try { fs.unlinkSync(preview); } catch(e) {}
                    try { fs.unlinkSync(f); } catch(e) {}
                }
                try { if (fs.existsSync(imgPath)) fs.unlinkSync(imgPath); } catch(e) {}
                if (!session.cutJobs) session.cutJobs = [];
                session.cutJobs.push({ buffers: buffers });
                const total = session.cutJobs.reduce((a, j) => a + j.buffers.length, 0);
                session.subState = null;
                await this.sendAndAutoRecall(groupId,
                    '已切割 ' + buffers.length + ' 张（累计 ' + total + ' 张）\n继续发图，或发「完成」选打印方式',
                    session.msgIds, 8000);
            } catch (error) {
                await this.sendAndAutoRecall(groupId, '切割错误: ' + error.message, session.msgIds, 10000);
                console.error('[Print] 手动切割错误:', error);
            }
            return true;
        }
        // 打印选择（WAITING_PRINT）
        if (session.subState === this.SUB.PRINT && (msg === '1' || msg === '2')) {
            try {
                const isDouble = msg === '2';
                const allBuffers = [];
                if (session.cutJobs) {
                    for (const job of session.cutJobs) {
                        for (const b of job.buffers) allBuffers.push(b);
                    }
                }
                await this.sendAndAutoRecall(groupId, (isDouble ? '双面' : '单面') + '打印... (共' + allBuffers.length + '张)', session.msgIds, 5000);
                if (isDouble) {
                    for (let i = 0; i < allBuffers.length; i += 2) {
                        if (i + 1 < allBuffers.length) {
                            await this.printDoubleSided(groupId, allBuffers[i], allBuffers[i + 1], session, isDemo);
                        } else {
                            await this.printImage(groupId, allBuffers[i], session, isDemo);
                        }
                    }
                } else {
                    for (const buf of allBuffers) {
                        await this.printImage(groupId, buf, session, isDemo);
                    }
                }
                if (session.msgIds && session.msgIds.length > 0) {
                    for (const id of session.msgIds) { await this.recallMessage(id); }
                    session.msgIds = [];
                }
                await this.recallUserMessages(userId);
                await this.sendAndAutoRecall(groupId, '打印完成', session.msgIds, 10000);
                this._afterPrintDone(c);
                this.sessions.delete(userId);
            } catch (error) {
                await this.sendAndAutoRecall(groupId, '打印错误: ' + error.message, session.msgIds, 10000);
                console.error('[Print] 打印错误:', error);
            }
            return true;
        }
        return false;
    }

    async _dispatchStitch(c) {
        const { msg, groupId, userId, session, isDemo } = c;
        // 布局选择（WAITING_LAYOUT）
        if (session.subState === this.SUB.LAYOUT) {
            if (!/^\d+$/.test(msg)) return false;
            const idx = parseInt(msg) - 1;
            if (idx < 0 || idx >= session.layouts.length) {
                await this.sendAndAutoRecall(groupId, '无效序号', session.msgIds, 10000);
                return true;
            }
            if (session.previewMsgIds) {
                for (let i = 0; i < session.previewMsgIds.length; i++) {
                    if (i !== idx) { await this.recallMessage(session.previewMsgIds[i]); }
                }
            }
            if (session.msgIds && session.msgIds.length > 0) {
                for (const id of session.msgIds) { await this.recallMessage(id); }
                session.msgIds = [];
            }
            await this.recallUserMessages(userId);
            const layout = session.layouts[idx];
            const pp = layout.rows * layout.cols;
            const pages = Math.ceil(session.buffers.length / pp);
            await this.sendAndAutoRecall(groupId, '打印... (' + layout.cols + 'x' + layout.rows + ')', session.msgIds, 5000);
            for (let p = 0; p < pages; p++) {
                const cnt = Math.min(pp, session.buffers.length - p * pp);
                const pageBuf = await this.stitchImages(session.buffers, layout.rows, layout.cols, p * pp, cnt);
                await this.printImage(groupId, pageBuf, session, isDemo);
            }
            session.subState = this.SUB.COMPLETED;
            await this.sendAndAutoRecall(groupId, '拼图完成，共' + pages + '页', session.msgIds, 10000);
            this.sessions.delete(userId);
            return true;
        }
        // 接收图片
        if (msg.includes('[CQ:image') || msg.includes('[CQ:file')) {
            if (!session.images) session.images = [];
            session.images.push(msg);
            await this.sendAndAutoRecall(groupId, '已收 ' + session.images.length + ' 张', session.msgIds, 10000);
            return true;
        }
        // 完成 → 生成排版
        if (msg === '完成') {
            const images = session.images || [];
            if (images.length === 0) {
                await this.sendAndAutoRecall(groupId, '还没有图片', session.msgIds, 10000);
                return true;
            }
            const buffers = [];
            for (const img of images) {
                try {
                    const officeName = img.match(/\[CQ:file,file=([^,\]]+)/);
                    if (officeName && isOfficeExt(officeName[1])) {
                        await this.sendAndAutoRecall(groupId, '拼图模式仅支持图片，文档请直接发送打印', session.msgIds, 10000);
                        continue;
                    }
                    const result = await this.downloadFile(img);
                    if (result.buffer && !result.isPdf) buffers.push(result.buffer);
                } catch (e) {}
            }
            if (buffers.length === 0) {
                await this.sendAndAutoRecall(groupId, '没有可用图片', session.msgIds, 10000);
                this.sessions.delete(userId);
                return true;
            }
            const layouts = this.getLayouts(buffers.length);
            let replyMsg = '[选择排版]\n';
            replyMsg += '-------------------\n';
            const previewMsgIds = [];
            for (let i = 0; i < layouts.length; i++) {
                const pp = layouts[i].rows * layouts[i].cols;
                const pages = Math.ceil(buffers.length / pp);
                replyMsg += (i+1) + '. ' + layouts[i].cols + 'x' + layouts[i].rows + '（' + pages + '页）\n';
                try {
                    const preview = await this.stitchImages(buffers, layouts[i].rows, layouts[i].cols, 0, Math.min(pp, buffers.length));
                    const previewMsgId = await this.sendPreview(groupId, preview, '方案' + (i+1) + '预览');
                    if (previewMsgId) previewMsgIds.push(previewMsgId);
                } catch (e) {
                    await this.sendAndAutoRecall(groupId, '方案' + (i+1) + '预览失败', session.msgIds, 5000);
                }
            }
            replyMsg += '-------------------\n';
            replyMsg += '回复序号打印';
            session.subState = this.SUB.LAYOUT;
            session.buffers = buffers;
            session.layouts = layouts;
            session.previewMsgIds = previewMsgIds;
            session.lastActive = Date.now();
            await this.recallUserMessages(userId);
            await this.sendAndAutoRecall(groupId, replyMsg, session.msgIds, 15000);
            return true;
        }
        return false;
    }

    // 解析一条消息内全部 [CQ:image]/[CQ:file] 段（支持一条消息带多个文件）
    _extractAllFileSegments(msg) {
        const segs = [];
        const re = /\[CQ:(image|file),file=([^,\]]+)(?:,url=([^\]]+))?\]/g;
        let m;
        while ((m = re.exec(msg)) !== null) {
            segs.push({ type: m[1], file: m[2], url: m[3] || null });
        }
        return segs;
    }

    // 极速打印入口：收集本条消息所有文件，并入该用户的瞬时批量（合并瞬间连发的多文件）
    async _instantPrint(c) {
        const items = this._extractAllFileSegments(c.msg);
        if (items.length === 0) return false;
        return await this._collectInstantBatch(c, items);
    }

    async _collectInstantBatch(c, items) {
        const { userId } = c;
        if (!this._instantBatches) this._instantBatches = new Map();
        let b = this._instantBatches.get(userId);
        if (!b) {
            b = { items: [], c: c, timer: null, flushing: false };
            this._instantBatches.set(userId, b);
        }
        b.c = c; // 刷新最新上下文（含当前 session/msgIds）
        for (const it of items) b.items.push(it);
        if (b.timer) clearTimeout(b.timer);
        // 1 秒防抖：连续转发进来在这个窗口内合并为一批，统一处理并给出进度
        b.timer = setTimeout(() => {
            b.timer = null;
            this._flushInstantBatch(userId);
        }, 1000);
        return true;
    }

    async _flushInstantBatch(userId) {
        const b = this._instantBatches.get(userId);
        if (!b) return;
        this._instantBatches.delete(userId);
        const { items, c } = b;
        if (!items.length) return;
        const { groupId, session, isDemo } = c;
        const total = items.length;
        try {
            if (total === 1) {
                await this.sendAndAutoRecall(groupId, '收到 1 个文件，处理中...', session.msgIds, 5000);
            } else {
                await this.sendAndAutoRecall(groupId, `收到 ${total} 个文件，开始批量处理...`, session.msgIds, 6000);
            }
            let okCount = 0;
            for (let i = 0; i < total; i++) {
                const it = items[i];
                const fname = String(it.file || '').split('/').pop().split('?')[0] || '文件';
                try {
                    const fakeMsg = `[CQ:${it.type},file=${it.file}${it.url ? ',url=' + it.url : ''}]`;
                    const result = await this.downloadFile(fakeMsg);
                    const label = result.isOffice ? '文档' : (result.isPdf ? 'PDF' : '图片');
                    const extra = result.isOffice ? ' 转换中...' : '';
                    await this.sendAndAutoRecall(groupId, `处理 ${i + 1}/${total}：${fname}（${label}）${extra}`, session.msgIds, 8000);
                    if (result.isPdf) {
                        await this.printPdf(groupId, result.buffer, session, isDemo);
                    } else {
                        await this.printImage(groupId, result.buffer, session, isDemo);
                    }
                    okCount++;
                    await this.sendAndAutoRecall(groupId, `✅ ${i + 1}/${total} 完成`, session.msgIds, 6000);
                } catch (err) {
                    await this.sendAndAutoRecall(groupId, `❌ ${i + 1}/${total} 失败：${err.message}`, session.msgIds, 15000);
                    console.error('[Print] 批量项失败', err);
                }
            }
            await this.sendAndAutoRecall(groupId, `✅ 批量完成 ${okCount}/${total}`, session.msgIds, 10000);
            this._afterPrintDone(c);
        } catch (e) {
            await this.sendAndAutoRecall(groupId, '错误: ' + e.message, session.msgIds, 10000);
            console.error(`[Print]`, e);
        }
    }

    _registerCommands() {
        const P = this;
        this.commands = {
            global: [
                { name: '菜单', match: c => c.msg === '菜单', handle: c => P._cmdMenu(c) },
                { name: '撤回', match: c => c.msg === '开启撤回' || c.msg === '关闭撤回', perm: 2, handle: c => P._cmdRecall(c) },
                { name: '监听此群', match: c => c.msg === '监听此群', perm: 2, handle: c => P._cmdListen(c, true) },
                { name: '不监听此群', match: c => c.msg === '不监听此群', perm: 2, handle: c => P._cmdListen(c, false) },
                { name: '开启演示', match: c => c.msg === '开启演示', perm: 2, handle: c => P._cmdDemo(c, true) },
                { name: '关闭演示', match: c => c.msg === '关闭演示', perm: 2, handle: c => P._cmdDemo(c, false) },
                { name: '帮助', match: c => c.msg === '帮助', handle: c => P._cmdHelp(c) },
                { name: '取消', match: c => c.msg === '取消' || c.msg === '退出', handle: c => P._cmdCancel(c) },
                { name: '状态', match: c => c.msg === '#打印状态', handle: c => P._cmdStatus(c) }
            ],
            allowed: [
                { name: '队列', match: c => c.msg === '队列', handle: c => P._cmdQueue(c) },
                { name: '队列选', match: c => P.queueSession && P.queueSession.userId === c.userId && /^\d+$/.test(c.msg), handle: c => P._cmdQueuePick(c) },
                { name: '队列退', match: c => P.queueSession && P.queueSession.userId === c.userId && c.msg === 'q', handle: c => P._cmdQueueQuit(c) },
                { name: '帮助', match: c => c.msg === '帮助', handle: c => P._cmdHelp(c) },
                { name: '取消', match: c => c.msg === '取消' || c.msg === '退出', handle: c => P._cmdCancel(c) },
                { name: '状态', match: c => c.msg === '#打印状态', handle: c => P._cmdStatus(c) }
            ],
            modeEntry: [
                { name: '双面', match: c => c.msg === '双面' || c.msg === '双面打印', handle: c => P._enterDuplex(c) },
                { name: '去黑', match: c => c.msg === '去黑' || c.msg === '去黑底', handle: c => P._enterBlack(c) },
                { name: 'a3', match: c => c.msg === 'a3' || c.msg === 'A3', handle: c => P._enterA3(c) },
                { name: '拼图', match: c => c.msg === '拼图', handle: c => P._enterStitch(c) }
            ]
        };
        this.modeDispatchers = {
            duplex: c => P._dispatchDuplex(c),
            black: c => P._dispatchBlack(c),
            a3: c => P._dispatchA3(c),
            stitch: c => P._dispatchStitch(c)
        };
    }

    // ==========================================
    // [R9 R4] Web 打印：printServer 复用入口（本地 HTTP 13012 调用）
    // ==========================================

    /**
     * 直接打印文件（不经群聊会话）：供 sea1/lib/printServer.js 复用。
     *  - PDF：直接走 CUPS `lp`（双面由 sides=two-sided-long-edge 实现）；
     *  - 图片：先 img2pdf（保留 rotation=ifvalid）再走 `lp`（复用现有管线）；
     *  - 参数：copies(1-20)/duplex(双面)/paper(media 纸张，白名单校验)。
     * @param {object} opts {filePath, copies, duplex, paper}
     * @returns {Promise<{ok:boolean, filePath:string, copies:number, duplex:boolean, paper:string, printer:string}>}
     */
    async printFile(opts) {
        opts = opts || {};
        const filePath = String(opts.filePath || '').trim();
        if (!filePath) throw new Error('filePath 必填');
        if (!fs.existsSync(filePath)) throw new Error('文件不存在: ' + filePath);
        const copies = Math.min(Math.max(parseInt(opts.copies, 10) || 1, 1), 20);
        const duplex = !!opts.duplex;
        const paper = /^[A-Za-z0-9_-]{1,16}$/.test(String(opts.paper || '')) ? String(opts.paper).toUpperCase() : 'A4';
        const fileExt = path.extname(filePath).toLowerCase();
        const isPdf = fileExt === '.pdf';
        const isOffice = !isPdf && OFFICE_EXTS.indexOf(fileExt) !== -1;
        const tmp = [];
        const tmpDirs = [];
        let target = filePath;
        try {
            if (isOffice) {
                // office 文档：soffice 转 PDF 后走 lp（双面由 lp 参数实现）
                const pdfPath = await this.convertOfficeToPdf(filePath);
                target = pdfPath;
                tmp.push(pdfPath);
                tmpDirs.push(path.dirname(pdfPath));
            } else if (!isPdf) {
                const pdfBuf = await this._pdfFromImageBuffer(fs.readFileSync(filePath));
                const tempPdf = path.join(this.tempDir, 'wp_' + Date.now() + '_' + Math.random().toString(16).slice(2, 8) + '.pdf');
                fs.writeFileSync(tempPdf, pdfBuf);
                if (!fs.existsSync(tempPdf) || fs.statSync(tempPdf).size < 1024) {
                    throw new Error('PDF生成失败');
                }
                target = tempPdf;
                tmp.push(tempPdf);
            }
            const lpOpts = ['-d', this.printerName, '-n', String(copies), '-o', 'fit-to-page', '-o', 'position=center', '-o', 'media=' + paper];
            if (duplex) lpOpts.push('-o', 'sides=two-sided-long-edge');
            const cmd = 'lp ' + lpOpts.map((x) => JSON.stringify(x)).join(' ') + ' ' + JSON.stringify(target);
            await execPromise(cmd, this.TO.proc);
            return { ok: true, filePath, copies, duplex, paper, printer: this.printerName };
        } finally {
            for (const f of tmp) {
                try { if (fs.existsSync(f)) fs.unlinkSync(f); } catch (e) { /* ignore */ }
            }
            for (const d of tmpDirs) {
                try { fs.rmSync(d, { recursive: true, force: true }); } catch (e) { /* ignore */ }
            }
        }
    }

}

module.exports = PrintPlugin;

// ==========================================
// [R9 R4] 模块级 printFile 导出（printServer 复用；懒加载单例，避免重复实例化）
// ==========================================
let _printSingleton = null;

/** 获取（并缓存）PrintPlugin 单例 */
function getPrintPlugin() {
    if (_printSingleton) return _printSingleton;
    let cfg = {};
    try {
        cfg = JSON.parse(fs.readFileSync(path.join(__dirname, '../../config.json'), 'utf8'));
    } catch (e) {
        cfg = {};
    }
    _printSingleton = new PrintPlugin('print', cfg);
    return _printSingleton;
}

/**
 * 模块级打印入口：加载配置后调用实例 printFile。
 * @param {object} opts {filePath, copies, duplex, paper}
 * @returns {Promise<{ok:boolean, ...}>}
 */
async function printFile(opts) {
    const inst = getPrintPlugin();
    await inst.loadConfig();
    return inst.printFile(opts);
}

module.exports.printFile = printFile;
module.exports.getPrintPlugin = getPrintPlugin;
module.exports.PrintPlugin = PrintPlugin;
